...
export default function SourcingList() {
  const { sourcing, mutateSourcing } = useSourcings();
  ...
  const deleteForm = async (form, formIdx) => {
    try {
      ...
      // remove locally
      const updatedForms = [...sourcing];
      updatedForms.splice(formIdx, 1);
      mutateSourcing(updatedForms);
    } catch (error) {
      console.error(error);
    }
  };
          ...
          (sourcing.length === 0 ? (
            ...
            <ul className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 place-content-stretch ">
              ...
              {sourcing
                ...
                .map((sourcing, sourcingIdx) => (
                  <li key={sourcing.id} className="relative h-56 col-span-1">
                    <div className="flex flex-col justify-between h-full bg-white rounded-md shadow">
                      <div className="p-6">
                        <p className="text-lg line-clamp-3">{sourcing.name}</p>
                      </div>
                      <Link href={`/sourcings/${sourcing.id}`}>
                      ...
                      <div className="divide-y divide-ui-gray-light ">
                        ...
                        <div className="flex justify-between px-4 py-2 text-right sm:px-6">
                          <p className="text-xs text-ui-gray-medium ">
                            {sourcing._count?.submissionSessions} responses
                          </p>
                          <Menu
                            as="div"
                                    ...
                                    <div className="py-1">
                                      <Menu.Item>
                                        {({ active }) => (
                                          <button
                                            onClick={() =>
                                              deleteForm(sourcing, sourcingIdx)
                                            }
