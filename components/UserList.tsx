...
import { UserCircleIcon, EllipsisHorizontalIcon, TrashIcon } from "@heroicons/react/24/solid";
...
export default function UserList() {
  ...
  const toggleRole = async (user ) => {;
    ...
    setLoading(true);
    setTimeout(async () => {
      const newUserRole = user.role === UserRole.ADMIN ? UserRole.PUBLIC : UserRole.ADMIN;
      const data = {
        id : user.id,
        role : newUserRole
      }
          ...
          ( (
            <ul className="flex flex-col">
              ...
              {users
                ...
                .map((user, userIdx) => (
                  <li key={userIdx} className="flex justify-between items-center border-y border-slate-100 py-2">
                    <div className="w-5/6 flex items-center ">
                      <UserCircleIcon className="w-12 text-slate-500" />
                      <div className="w-1/5 mx-3 flex justify-between items-center text-lg line-clamp-3 text-slate-900"><p className="w-1/2 font-light" >{user.firstname}</p> <p className="w-1/2 font-bold">{user.lastname}</p></div>
                    ...
                    <div className="flex items-center justify-between w-1/6">
                      ...
                      >
                        <Switch.Label
                          as="span"
                          className={user.role === UserRole.ADMIN ? "text-sm font-bold text-red-600 mr-3" : "text-sm font-bold text-gray-900 mr-3"}
                          passive={true}
                        >
                          {user.role === UserRole.ADMIN ? UserRole.ADMIN : UserRole.PUBLIC}
                        </Switch.Label>
                        {loading ? (
                        ...
                        <Switch
                          ...
                          onChange={(e) => toggleRole(user)}
                          
                          className={classNames(
                            user.role === UserRole.ADMIN ? "bg-red-600 transition-colors ease-in-out duration-200" : "bg-gray-200",
