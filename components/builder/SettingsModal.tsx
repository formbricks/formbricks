...
export default function SettingsModal({open, setOpen, formId}) {
    ...
    const toggleClose = async () => {
        ...
        setTimeout(async () => {
            ...
            setLoading(false)
            toast(newNoCodeForm.closed ? "Your form is now closed for submissions " : "Your form is now open for submissions 🎉");
        }, 500);
    };
            ...
            <Dialog as="div" className="relative z-10" onClose={setOpen}>
                ...
                <div className="fixed inset-0 z-10 overflow-y-auto">
                    <div className="flex items-end justify-center min-h-full p-4 text-center sm:items-center sm:p-0">
                        ...
                        >
                            <Dialog.Panel
                                className="relative px-4 pt-5 pb-4 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:max-w-4xl sm:w-full sm:p-6">
                                    ...
                                    <div className="mt-2 text-sm text-gray-500 w-full">
                                        <Switch.Group as="div" className="flex items-center justify-between w-full">
                                            <span className="flex-grow flex flex-col">
                                                ...
                                                >
                                                    Your form is currently <span className="font-bold">{noCodeForm.closed ? 'closed' : 'open'}</span> for submissions.
                                                </Switch.Description>
                                            </span>
                                            {loading ? <TailSpin color="#1f2937" height={30} width={30}/> : (
                                                <Switch
                                                    checked={noCodeForm.closed}
                                                    onChange={() => toggleClose()}
                                                    className={classNames(
                                                        noCodeForm.closed ? 'bg-indigo-600' : 'bg-gray-200',
