...
1. Follow the same naming pattern as we have in other services
   1. If using Prisma’s `findUnique` then the name should be `get` + `ServiceName` (in singular), e.g `getApiKey`
   2. If using Prisma’s `findMany` then the name should be `get` + `ServiceName` (in plural), e.g `getApiKeys`
   3. If your function's primary purpose is to retrieve or manipulate data based on a specific attribute or property of a resource, use "`by`" followed by the attribute name. For example:
   ...
   4. If using Prisma’s `create` then `createApiKey`
   5. If using Prisma’s `update` then `updateApiKey`
   6. if using Prisma’s `delete` then `deleteApiKey`
