...
- Handle authentication and CORS in management API’s
...
We have two API’s: Management API and Client API.

The public endpoints of the Client API are used by the link survey and `formbricks-js` to send responses and displays to formbricks. Client API’s can be found in `apps/web/app/api/v1/client`

The Management API offers the same functionality as the management frontend and can be used to create surveys, view responses or change account settings. The endpoints require an api key that the user can obtain in the management frontend. Management API’s can be found in `apps/web/app/api/v1/management`.

Please keep the following in mind:

- When dealing with Management API’s always make sure to require authentication via API keys and a sufficient authorization check.
