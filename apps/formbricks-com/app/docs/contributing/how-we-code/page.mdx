...
- Read environment variables from `.env.ts`
...
Environment variables (`process.env`) shouldn’t be accessed directly but be added in the `.env.ts` and should be accessed from here. This practice helps us ensure that the variables are typesafe.
