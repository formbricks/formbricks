...
<Note>
For a smooth experience, we suggest the above cloue IDE methods.
...
3. Create a `.env` file based on `.env.example`. It's already preset to work with the docker-compose setup but you can also change values if needed.
