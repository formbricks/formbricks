...
<Note>If you run into “No such container”, use `docker ps` to find your container name, e.g. `formbricks_postgres_1`.</Note>

<Note>If you prefer storing the backup as an `*.sql` file remove the `-Fc` (custom format) option. In case of a restore scenario you will need to use `psql` then with an empty `formbricks` database.</Note>
...
docker run --rm \
  ...
  ghcr.io/formbricks/data-migrations:v1.6.0
```
...
<Note>Currently the package needs to be pinned to `~1.6.5`, see https://github.com/formbricks/formbricks/issues/2273</Note>
