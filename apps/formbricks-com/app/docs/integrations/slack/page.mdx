...
3. You will now be taken to the Slack OAuth page where you can select the Slack account you want to use for the integration and click on the "Allow" button.
...
4. Go to the **OAuth & Permissions** tab on the sidebar and add the following **Bot Token Scopes**: `chat:write` and `incoming-webhook`. You can add them by clicking on **Add an OAuth Scope**.
5. Add the **Redirect URLs** under **OAuth & Permissions** tab. You can add the following URLs:

   - If you are running formbricks locally, you can enter `https://localhost:3000/api/auth/callback/slack`.
   - Or, you can enter `https://<your-public-facing-url>/api/auth/callback/slack`
...
2. Select "Manage Channel Stream" button in the Slack card.
