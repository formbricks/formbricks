...
### Manually Rendering the Survey Modal

For advanced users, trigger the survey modal by providing the survey ID and ensuring its status is in-progress. Please note that this method disregards other triggering configurations, such as URL, actions, user attributes, display percentage, and recontact options. Only the survey delay setting is applied.
...
Replace `survey_id` with the survey id found at the bottom of the survey summary page.
