...
const ProductItem = ({ label, usersCount, price, onSliderChange }) => (
  <div>
    <div className="mb-2 flex items-center gap-x-2 md:gap-x-4">
      <div className="md:text-md w-3/6 text-left text-sm font-medium text-slate-700">{label}</div>
      <div className="md:text-md w-2/6 text-center text-sm font-medium text-slate-700">
        {Math.round(usersCount).toLocaleString()} MTU
      </div>
      <div className="md:text-md flex w-1/6 items-center justify-end text-center text-sm font-medium text-slate-700 md:justify-center">
    ...
    <div className="my-2 w-5/6 pr-8 md:pr-20">
      ...
      <div className="mt-2 flex items-center justify-between text-sm">
        {[3, 4, 5, 6].map((mark) => (
          <span key={mark} className="text-slate-600">
...
const Headers = () => (
  <div className="mb-4 flex justify-between">
    <h3 className="text-base font-semibold text-slate-700 md:text-lg">Product</h3>
    <h3 className="w-1/6 text-center text-base font-semibold text-slate-700 md:text-lg">Subtotal</h3>
  </div>
);

const MonthlyEstimate = ({ price }) => (
  <div className="mt-2 flex justify-between">
    <span className="text-base font-semibold text-slate-700 md:text-lg">Monthly estimate:</span>
    <div className="w-1/6 text-center">
      <span className="w-1/6 text-base text-slate-700 md:text-lg md:font-semibold">${price.toFixed(2)}</span>
      <span className="hidden text-sm text-slate-400 md:block md:text-base"> / month</span>
    </div>
  </div>
);
