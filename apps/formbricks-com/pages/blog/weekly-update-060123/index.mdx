...
## PR at [Cal.com](http://Cal.com) repo for Feedback Box

We’ve embedded the prototype of the feedback box into Cal.com and opened a PR to get feedback from the team. We aren’t satisfied with the result yet: it still feels like an alien element in the UI. To tackle that, we start working on a task we planned for the next use case: seperating the form content from the wrapper (see next point).

[CAL.COM](http://CAL.COM) FEEDBACK BOX SCREENSHOT - Link to PR?
