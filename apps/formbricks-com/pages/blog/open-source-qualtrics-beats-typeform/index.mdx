...
_In September, we kicked it off with a Typeform open-source alternative. As we build and learn, our focus is shifting. Here’s why:_

In this article we will talk about how we look at form and survey tools today, why experience management not only matters for enterprise and why the endgame looks a lot more like open-source Qualtrics. Qualtrics? What happened to Typeform?
...
All of the above looks good. And yet, we decided to move away from a general purpose no-code form builder. To understand why we have to look at three aspects and how they influence each other: 1) The appeal of OSS, 2) the people we attracted and 3) the value Typeform offers. Let’s lay out the facts first:
...
Taking all the above into account, it becomes clear that simply rebuilding Typeform open-source is not a venture-scale idea. The Commercial Open-Source Software playbook kinda goes like this: Grow with community and then sell into enterprise bottom up.

The first part is doable: Grow a community with a largely free and open-source Typeform clone. You might even make money offering a hosted solution but there is an obvious cap, when Tally.so offers 95% of Typeforms features for free - and then charges $29 / month.
...
Qualtrics took the business objective of their customers and productized an opinionated, research-based solution that excels at analyzing large amounts of qualitative data in a way that guides decision-making. Typeform makes pretty forms. It’s no surprise that especially large companies are willing to pay $5000+ / month for Qualtrics while you hear complaints about the $49 / month Typeform charges all too often.
...
Experience management covers four areas of experiences: Customer, Employee, Product and Brand. To be able to digg deep, we decided to focus on Product Experiences as the other three are less relevant when the product sucks.
...
- Fits into current product / growth stack: Sits between product analytics, CRM and messaging systems like customer.io
...
### In practice: What have we built over the past month?
...
So we built that. We built a visual form creator, event tracking, user attribute management and an analysis dashboard built for SaaS products. Users can implement Formbricks via NPM or by copying a snippet into the head of their products HTML.
