...
```bash
// Go to the packages/js directory
cd packages/js
...
However, in our experience it's better to run `pnpm dev` than having to terminals open and one with the Formbricks app and one with the demo.
