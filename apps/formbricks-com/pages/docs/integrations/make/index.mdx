...
<Callout title="Nail down your survey first" type="note">
  Any changes in the survey cause additional work in the _Scenario_. It makes sense to first settle on the
  survey you want to run and then get to setting up Make.
</Callout>

## Step 1: Setup your survey incl. `questionId` for every question

When setting up the _Scenario_ on Make your life will be easier when you change the `questionId`s of your survey questions. You can only do so **before** you publish your survey.
...
You are asked to enter a Formbricks API key. Find out how to get one here in the [API Key tutorial](/docs/api/api-key-setup).
...
In order to set up Make you’ll need a test response in the selected survey. This allows you to select the individual values of each response in your Scenario. If you have Formbricks running locally and you want to set up an in-app survey, you can use our [Demo App](/docs/contributing/demo) to trigger a survey and submit a response.
...
Click on "Add module" and select the Google Sheet app:

<Image src={AddModule} alt="Search Formbricks" quality="100" className="rounded-lg" />{" "}

Then, select an action that you want to perform. Here we choose "Add a Row":

<Image src={SelectAction} alt="Search Formbricks" quality="100" className="rounded-lg" />{" "}

You are prompted to enter the details of the spreadsheet and map table fields to fill in your Formbricks data. As you can see in the screenshot, you can fill in individual data points into the cells of your choice:

<Image src={SelectFields} alt="Search Formbricks" quality="100" className="rounded-lg" />{" "}

Now, whenever a new response is created, a new row will be added to the spreadsheet we selected:
