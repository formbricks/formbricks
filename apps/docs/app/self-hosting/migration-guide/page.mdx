...
```bash
docker exec formbricks-quickstart-postgres-1 pg_dump -Fc -U postgres -d formbricks > formbricks_pre_v 2.0_$(date +%Y%m%d_%H%M%S).dump
```
...
From this upgrade, we now **dynamically fetch** the package from our API endpoint so that you always have the latest version of the package (v 2.0.0+).
