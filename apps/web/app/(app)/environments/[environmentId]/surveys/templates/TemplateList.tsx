...
export default function TemplateList({
  ...
  const [searchFilteredTemplates, setSearchFilteredTemplates] = useState<Array<any> | null>(null);
  ...
  const handleSearch = (e) => {
    ...
    const filteredTemplates = templates.filter((template) => {
      return (
        template.name.toLowerCase().includes(searchInputValue) ||
        template.description.toLowerCase().includes(searchInputValue)
      );
    });
    setSearchFilteredTemplates(filteredTemplates);
  };
      ...
      <main className="relative z-0 flex-1 overflow-y-auto px-6 pb-6 pt-3 focus:outline-none">
        <div className="mb-6 flex flex-wrap gap-1">
          <input
            ...
            className="w-full rounded-lg p-2 transition duration-500 focus:scale-[1.04] focus:drop-shadow-[0_35px_35px_rgba(4,330,33,0.25)]"></input>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          {searchQuery
            ? searchFilteredTemplates?.map((template: TTemplate) => (
                <div
                  onClick={() => {
