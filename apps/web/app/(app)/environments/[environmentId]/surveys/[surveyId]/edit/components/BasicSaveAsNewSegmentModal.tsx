...
import { createBasicSegmentAction, updateBasicSegmentAction } from "../actions";
...
const BasicSaveAsNewSegmentModal: React.FC<SaveAsNewSegmentModalProps> = ({
  open,
  setOpen,
  localSurvey,
  segment,
  ...
  const handleSaveSegment: SubmitHandler<SaveAsNewSegmentModalForm> = async (data) => {
    ...
    try {
      ...
      if (!!segment && segment?.isPrivate) {
        const updatedSegment = await updateBasicSegmentAction(segment.environmentId, segment.id, {
          ...segment,
          title: data.title,
      ...
      const createdSegment = await createBasicSegmentAction({
...
export default BasicSaveAsNewSegmentModal;
