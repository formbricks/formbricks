...
export default function SummaryMetadata({
  ...
  const completionRate = !responses ? 0 : (completedResponses / responses.length) * 100;

  return (
    <>
      <div className="mb-4 ">
        <div className="flex flex-col-reverse gap-y-2 lg:grid lg:grid-cols-3 lg:gap-x-2">
          <div className="grid grid-cols-2 gap-4 md:grid md:grid-cols-5 md:gap-x-2 lg:col-span-2">
            ...
            <div className="flex flex-col justify-between space-y-2 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-sm text-slate-600">Drop Offs %</p>
              <p className="text-2xl font-bold text-slate-800">
                {survey.analytics.numDisplays === 0 ? (
                  <span>-</span>
                ) : (
                  <span>
                    {(
                      ((survey.analytics.numDisplays - completedResponses) / survey.analytics.numDisplays) *
                      100
                    ).toFixed(2)}
                  </span>
                )}
