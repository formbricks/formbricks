...
export const ActivityItemIcon = ({
  ...
  const Count = () => <div className="text-lg font-bold">{count}x</div>;
      ...
      {activityItem && (
        <div className="h-12 w-12 rounded-full bg-white p-3 text-slate-500  duration-100 ease-in-out group-hover:scale-110 group-hover:text-slate-600">
          {activityItem.type === "attribute" ? (
            <>{displayOnlyIcon ? <TagIcon /> : <Count />}</>
          ) : activityItem.type === "display" ? (
            <>{displayOnlyIcon ? <EyeIcon /> : <Count />}</>
          ) : activityItem.type === "event" ? (
            <div>
              {activityItem.actionType === "code" && <>{displayOnlyIcon ? <CodeBracketIcon /> : <Count />}</>}
              {activityItem.actionType === "noCode" && (
                <>{displayOnlyIcon ? <CursorArrowRaysIcon /> : <Count />}</>
              )}
              {activityItem.actionType === "automatic" && (
                <>{displayOnlyIcon ? <SparklesIcon /> : <Count />}</>
              )}
            </div>
          ) : (
            <QuestionMarkCircleIcon />
        ...
        <div>
          <div className="font-semibold text-slate-700">
            {activityItem.type === "attribute" ? (
              ...
              <p>{activityItem.displaySurveyName || "Seen survey"}</p>
            ) : activityItem.type === "event" ? (
