...
export const MainNavigation = ({
  ...
  // Force sidebar expanded in settings mode — icons-only doesn't work for settings nav
  const effectiveIsCollapsed = isSettingsMode ? false : isCollapsed;
  const effectiveIsTextVisible = isSettingsMode ? false : isTextVisible;
  ...
  const switcherTriggerClasses = cn(
    ...
    effectiveIsCollapsed ? "flex items-center justify-center" : ""
  );
  ...
  return (
    <>
      {workspace && (
        <aside
          className={cn(
            "z-40 flex flex-col justify-between overflow-x-hidden rounded-r-xl border-r border-slate-200 bg-white pt-3 shadow-md transition-all duration-100",
            effectiveIsCollapsed ? "w-sidebar-expanded" : "w-sidebar-collapsed"
          )}>
          <div className={cn(isSettingsMode && "flex flex-1 flex-col overflow-hidden")}>
              ...
              <div className="flex items-center justify-between px-3 pb-4">
                {isSettingsMode ? (
                  <Link
                    ...
                    className={cn(
                      "flex items-center gap-1.5 text-sm font-medium text-slate-600 transition-opacity duration-100 hover:text-slate-900",
            ...
            {isSettingsMode ? (
              ...
              <SettingsSidebarContent
                ...
                isTextVisible={effectiveIsTextVisible}
                workspaces={workspaces}
                isLoadingWorkspaces={isLoadingWorkspaces}
                onWorkspaceChange={handleWorkspaceChange}
                onWorkspaceDropdownOpen={() => {
                  if (!isWorkspaceDropdownOpen && workspaces.length === 0 && !workspaceLoadError) {
                    loadWorkspaces();
                  }
                }}
                ...
                onOrganizationDropdownOpen={() => {
                  if (!isOrganizationDropdownOpen && organizations.length === 0 && !organizationLoadError) {
                    loadOrganizations();
                  }
                }}
              />
          ...
          {!isSettingsMode && (
            <div>
              ...
              {!effectiveIsCollapsed && isFormbricksCloud && trialDaysRemaining !== null && (
                <Link
                  href={`/workspaces/${workspace.id}/settings/organization/billing`}
                  className="m-2 block">
