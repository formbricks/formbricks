...
const ContentLayout = ({ headline, description, children }) => {
    return (
        <div className="flex h-screen">
            <div className="m-auto text-center flex flex-col gap-7 text-slate-700">
                <h2 className="text-3xl font-bold ">{headline}</h2>
                <p className="text-2xl  ">{description}</p>
                <div className="text-xs flex gap-5 justify-center">
                    {children}
                </div>
            </div>
        </div>
...
export const NotLoggedInContent = ({ email, token, redirectUrl }) => {
    ...
    return (
        ...
        >
            <Button variant="secondary" href={`/auth/signup?inviteToken=${token}&email=${email}`}>Create account</Button>
            <Button variant="primary" href={`/auth/login?callbackUrl=${redirectUrl}&email=${email}`}>Login</Button>
        </ContentLayout>
    );
};
...
export const WrongAccountContent = () => {
    return (
        ...
        >
            ...
            <Button variant="primary" href="/">Go to app</Button>
        </ContentLayout>
    );
};
