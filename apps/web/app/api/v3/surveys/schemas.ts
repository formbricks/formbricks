...
function normalizeMetadata(value: unknown, defaultLanguage: string): unknown {
  ...
  return {
    ...value,
    ...(value.title !== undefined ? { title: normalizePublicI18nField(value.title, defaultLanguage) } : {}),
    ...(value.description !== undefined
      ? { description: normalizePublicI18nField(value.description, defaultLanguage) }
      : {}),
  };
}
...
function normalizeV3SurveyDocumentInput(
  value: unknown,
  options: { fallbackDefaultLanguage: string; applyDefaultLanguage: boolean }
): unknown {
  if (!isPlainObject(value)) {
    return value;
  }
  ...
  return {
    ...
    ...(value.defaultLanguage === undefined
      ? options.applyDefaultLanguage
        ? { defaultLanguage: options.fallbackDefaultLanguage }
        : {}
      : { defaultLanguage: normalizedDefaultLanguage ?? value.defaultLanguage }),
    ...(value.metadata !== undefined
      ? { metadata: normalizeMetadata(value.metadata, defaultLanguageForI18n) }
      : {}),
    ...(value.welcomeCard !== undefined
      ? {
          welcomeCard: normalizePublicI18nFields(
            value.welcomeCard,
...
function isElementTypeWithStrictKeys(type: string): type is ElementTypeWithStrictKeys {
  return Object.prototype.hasOwnProperty.call(ELEMENT_KEYS_BY_TYPE, type);
}
...
function addUnknownKeyIssues(
  ...
  const allowedFields = context ? Array.from(allowedKeys).sort().join(", ") : "";
...
function validateTranslatableField(
  ...
  for (const key of Object.keys(value)) {
    ...
    if (defaultLanguage && normalizedKeyLookup === defaultLanguage.toLowerCase()) {
...
function validateElement(
  ...
  const elementKeys =
    isPlainObject(value) && typeof value.type === "string"
      ? isElementTypeWithStrictKeys(value.type)
        ? ELEMENT_KEYS_BY_TYPE[value.type]
        : ELEMENT_KEYS
      : ELEMENT_KEYS;
  ...
  [
    ...
    "ctaButtonLabel",
    "html",
  ].forEach((key) => validateTranslatableField(value[key], `${path}.${key}`, issues, defaultLanguage));
  [
    "addressLine1",
    "addressLine2",
    "city",
    ...
    "phone",
    "company",
  ].forEach((key) => validateToggleInput(value[key], `${path}.${key}`, issues, defaultLanguage));
...
function validateEnding(
  ...
  ["headline", "subheader", "buttonLabel"].forEach((key) =>
...
const ZV3SurveyDocumentObject = z
  .object({
    name: z.string().trim().min(1, "Survey name is required"),
    ...
    blocks: ZSurveyBlocks.min(1, "At least one block is required"),
...
function createV3PatchSurveyBodyBase(defaultLanguage: string) {
  return z.preprocess(
    ...
    z
      .object({
        name: z.string().trim().min(1, "Survey name is required").optional(),
        ...
        blocks: ZSurveyBlocks.min(1, "At least one block is required").optional(),
