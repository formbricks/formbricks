...
export async function POST(req: NextRequest): Promise<NextResponse> {
  const { fileName, contentType, environmentId, allowedFileExtensions } = await req.json();
  ...
  return await uploadPublicFile(fileName, environmentId, contentType);
}

const uploadPublicFile = async (fileName: string, environmentId: string, contentType: string) => {
  ...
  try {
    const signedUrl = await getSignedUrlForS3Upload(fileName, contentType, accessType, environmentId, true);
