...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const { surveys, noCodeActions, brandColor } = await getSettings(user.environmentId, user.id);

  captureTelemetry("session created");

  return responses.successResponse({ surveys, noCodeActions, brandColor }, true);
}
