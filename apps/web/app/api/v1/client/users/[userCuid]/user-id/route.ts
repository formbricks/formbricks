...
export async function POST(request: Request, params: { userCuid: string }): Promise<NextResponse> {
  ...
  const { surveys, noCodeActions, brandColor } = await getSettings(environmentId, returnedUser.id);

  return responses.successResponse({ user: returnedUser, surveys, noCodeActions, brandColor }, true);
}
