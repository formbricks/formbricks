...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const { surveys, noCodeActions, brandColor } = await getSettings(environmentId, user.id);

  return responses.successResponse({ user, session, surveys, noCodeActions, brandColor }, true);
}
