import { type TSurveyDateStorageFormat, parseDateByFormat } from "@formbricks/surveys/date-format";
...
const DATE_FORMATS: TSurveyDateStorageFormat[] = ["y-M-d", "d-M-y", "M-d-y"];

function parseStoredDateString(value: string): Date | null {
    ...
    // Then check if it matches response data
      ...
      if (value) {
        if (typeof value === "string") {
          const parsedDate = parseStoredDateString(value);
          if (parsedDate !== null) {
