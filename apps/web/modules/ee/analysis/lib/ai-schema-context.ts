...
## Guidelines
- First decide whether the request can be answered from this feedback data at all. Set \`answerable\` to false when it is gibberish (random characters, keyboard mashing) or asks about something the data does not hold (the weather, a joke, general knowledge, writing code). A vague request that is still about the feedback ("show me something useful", "how are we doing") is answerable. When \`answerable\` is false, the rest of the query is ignored — do not invent one.
