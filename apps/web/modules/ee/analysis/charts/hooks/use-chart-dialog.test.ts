...
describe("useChartDialog", () => {
  ...
  describe("handleChartTypeChange - prefill", () => {
    test("pre-populates the sentiment query when the sentiment type is picked", async () => {
      const { result } = renderHook(() => useChartDialog(baseProps));

      await act(async () => {
        result.current.handleChartTypeChange("sentiment");
      });

      expect(result.current.selectedChartType).toBe("sentiment");
    ...
    test("leaves shape-only chart types without a prefill", async () => {
      const { result } = renderHook(() => useChartDialog(baseProps));
      ...
      expect(result.current.prefillQuery).toBeUndefined();
    });

    test("keeps a query that already reads sentiment", async () => {
      const { result } = renderHook(() => useChartDialog(baseProps));
    ...
    test("clears the prefill once the prefilled query has run", async () => {
      const { result } = renderHook(() => useChartDialog(baseProps));

      await act(async () => {
        result.current.handleChartTypeChange("sentiment");
      });
      expect(result.current.prefillQuery).toBeDefined();
      ...
      await act(async () => {
        result.current.handleChartGenerated({
          ...
          chartType: "sentiment",
          data: [{ "FeedbackRecords.sentiment": "positive", "FeedbackRecords.count": 4 }],
        });
      });
