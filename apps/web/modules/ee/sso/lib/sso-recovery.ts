 ...
 * - The `where` is scoped by `userId`, NOT by `providerAccountId`, even though Better Auth's own
...
const reclaimUnverifiedLocalAuthIfNeeded = async ({
  ...
  //
  // ACCESS tokens are NOT revoked on this release branch: `oauthAccessToken` has no `revoked` column
  // here, so there is nothing to write. On `main` that write exists, and it is worth being clear that it
  // buys almost nothing anyway — our config sets `resources` and never sets `disableJwtPlugin`, so
  // `isJwtAccessToken` is always true and every access token is a self-contained JWT that
  // `createJwtAccessToken` signs without persisting, and `/api/mcp` verifies bearers against JWKS
  // (`modules/mcp/auth.ts`) without reading that table at all. Upstream's own revoke endpoint says as
  // much — "JWT access tokens are self-contained and cannot be revoked server-side". So the behavioural
  // residual is identical to `main`: a squatter's JWT stays valid for up to `accessTokenExpiresIn`
  // (15 min) after recovery. Shortening that, or checking revocation at the resource server, is the only
