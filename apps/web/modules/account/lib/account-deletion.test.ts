...
const expectCalledBefore = (firstCall: ReturnType<typeof vi.fn>, secondCall: ReturnType<typeof vi.fn>) => {
  expect(firstCall.mock.invocationCallOrder[0]).toBeLessThan(secondCall.mock.invocationCallOrder[0]);
};
...
describe("deleteUserWithAccountDeletionAuthorization", () => {
  ...
  test("requires the completed SSO reauthentication marker by default", async () => {
    ...
    expectCalledBefore(mocks.getUser, mocks.consumeAccountDeletionSsoReauthentication);
    expectCalledBefore(mocks.consumeAccountDeletionSsoReauthentication, mocks.deleteUser);
    expect(mocks.deleteUser).toHaveBeenCalledWith(user.id);
  });

  test("can bypass SSO reauthentication for passwordless SSO users when explicitly configured", async () => {
    const { deleteUserWithAccountDeletionAuthorization } = await loadAccountDeletionModule({
