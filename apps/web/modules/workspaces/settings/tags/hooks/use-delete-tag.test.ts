...
describe("useDeleteTag", () => {
  ...
  test("deletes through the tag route, then invalidates exactly this workspace's tag list", async () => {
    ...
    const { result } = renderHook(() => useDeleteTag("ws_1"), { wrapper: wrapper(queryClient) });
    result.current.mutate({ tagId: "cltt1234567890123456789012" });
  ...
  test("surfaces a rejected request instead of reporting success", async () => {
    ...
    await expect(result.current.mutateAsync({ tagId: "cltt1234567890123456789012" })).rejects.toThrow(
      "not yours"
    );
