...
export const getMetadataForLinkSurvey = async (surveyId: string): Promise<Metadata> => {
  ...
  const timeToFinish = survey.welcomeCard.timeToFinish
    ? encodeURIComponent(calculateTimeToComplete(survey.questions, survey.endings))
    : "";
  const ogImgURL = `/api/v1/og?brandColor=${brandColor}&name=${surveyName}&timeToFinish=${timeToFinish}`;
