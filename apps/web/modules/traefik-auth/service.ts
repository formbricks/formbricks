...
export const authorizeTraefikRequest = async (request: NextRequest): Promise<Response> => {
  ...
  // allowed as. ext_authz has no use for the principal: Traefik reads an allow from the status line, so
  ...
  return outcome.status === "allow" ? buildTraefikAllowResponse() : outcome.response;
};
