...
export const card = ({ title, description, body, bodyClass = "px-4 pt-4", extra = "" }) => `
 ...
 */
export const tableBefore = ({ heads, rows, headRowClass = "bg-slate-100", bodyClass = "" }) => `
...
<div class="relative overflow-auto">
  <table class="w-full caption-bottom text-sm">
    ...
    <tbody class="[&_tr:last-child]:border-0 ${bodyClass}">
      ${rows
        .map(
          (cells) => `<tr class="${cn("border-b border-slate-200 bg-white transition-colors", cells.rowCls)}">
        ${cells.cells.map((c) => `<td class="${cn("p-4 align-middle", c.cls)}">${c.html}</td>`).join("")}
      </tr>`
        )
        .join("")}
