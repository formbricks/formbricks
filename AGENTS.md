...
Every PR must use `.github/pull_request_template.md`. Fill every section from the actual diff: link the Linear ticket (or explain its absence), state in **How this was tested** what you actually ran and verified with real results, attach screenshots or a recording for any UI change, and complete the `## QA / Test Plan` section — the source of truth for release QA (follow the guidance inline in the template; keep its subsection headings). Fill it on PR open and re-update it in the same turn on every PR change (new commits, scope or review fixes) so it never drifts from the diff — treat a stale section as a bug.
