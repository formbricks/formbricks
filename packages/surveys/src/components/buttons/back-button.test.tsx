...
describe("BackButton", () => {
  ...
  test('renders with default text "Back" and default tabIndex when no label is provided', () => {
    const onClick = vi.fn();
    const { getByRole } = render(<BackButton onClick={onClick} />);
    const button = getByRole("button");
    expect(button).toBeDefined();
    expect(button.textContent?.trim()).toEqual("buttons.back");
    expect(button.tabIndex).toEqual(2);
  });

  test("renders with the provided backButtonLabel", () => {
    const onClick = vi.fn();
    const label = "Go Back";
  ...
  test('has dir="auto" and type="button" attributes', () => {
