...
export function PictureSelectionElement({
  ...
  const getButtonClassName = (choiceId: string): string => {
    ...
    return cn(
      "fb:relative fb:w-full fb:cursor-pointer fb:overflow-hidden fb:border fb:rounded-custom fb:focus-visible:outline-hidden fb:focus-visible:ring-2 fb:focus-visible:ring-brand fb:focus-visible:ring-offset-2 fb:aspect-4/3 fb:min-h-28 fb:max-h-[50vh] group/image",
      isSelected ? "fb:border-brand fb:text-brand fb:z-10 fb:border-4 fb:shadow-xs" : ""
    );
  };
    ...
    <form key={element.id} onSubmit={handleFormSubmit} className="fb:w-full">
      ...
      <div className="fb:mt-4">
        <fieldset>
          ...
          <div className="fb:bg-survey-bg fb:relative fb:grid fb:grid-cols-1 fb:sm:grid-cols-2 fb:gap-4">
