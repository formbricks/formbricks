...
const getPreviousResultsCacheKeyAndTag = (hashedKey: string) => `getPreviousResult-${hashedKey}` as const;
...
const getPreviousResult = (hashedKey: string): Promise<{ active: boolean | null; lastChecked: Date }> =>
  cache(
    async () => ({
      active: null,
      lastChecked: new Date(0),
    }),
    [getPreviousResultsCacheKeyAndTag(hashedKey)],
    {
      tags: [getPreviousResultsCacheKeyAndTag(hashedKey)],
    }
  )();
...
const setPreviousResult = async (
  hashedKey: string,
  previousResult: { active: boolean | null; lastChecked: Date }
) => {
  revalidateTag(getPreviousResultsCacheKeyAndTag(hashedKey));
...
export const getIsEnterpriseEdition = async (): Promise<boolean> => {
  ...
  if (E2E_TESTING) {
    const previousResult = await getPreviousResult(hashedKey);
    if (previousResult.lastChecked.getTime() === new Date(0).getTime()) {
      // first call
      await setPreviousResult(hashedKey, { active: true, lastChecked: new Date() });
      return true;
  ...
  if (previousResult.active === null) {
    if (isValid === null) {
      await setPreviousResult(hashedKey, { active: false, lastChecked: new Date() });
      return false;
    }
  }

  if (isValid !== null) {
    await setPreviousResult(hashedKey, { active: isValid, lastChecked: new Date() });
    return isValid;
  } else {
