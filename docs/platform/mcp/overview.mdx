...
| `feedbackRecords:read` | `list_feedback_datasets`, `list_feedback_records`, `get_feedback_record` | Read feedback records from a workspace's feedback dataset |
...
<Note>
  ...
  (`feedbackDirectories`), plus a feedback dataset assigned to the workspace. Without the
