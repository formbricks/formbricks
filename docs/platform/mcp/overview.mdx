...
| `feedbackRecords:read` | `list_feedback_directories`, `list_feedback_records`, `get_feedback_record` | Read feedback records from a workspace's feedback directory |
| `feedbackRecords:write` | `create_feedback_record` | Create feedback records |
...
<Note>
  ...
  (`feedbackDirectories`), plus a feedback directory assigned to the workspace. Without the
  entitlement or a directory, those tools return an authorization or validation error while the
  survey tools keep working.
</Note>
