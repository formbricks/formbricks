...
<AccordionGroup>
  ...
  <Accordion title="Self-hosted: the browser can't complete sign-in / redirects fail">
    ...
    `MCP_OAUTH_JWKS_URL` to the internal `/api/auth/jwks` endpoint. Keep `WEBAPP_URL` and `BETTER_AUTH_URL`
    public: the internal override is only used for the server-side signing-key fetch.
