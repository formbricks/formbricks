...
<Steps>
  ...
  <Step title="Authenticate">
    ...
    ```bash
    codex mcp login formbricks --scopes surveys:read,workflows:read,feedbackRecords:read,offline_access
    ```
...
(`surveys:write`, `workflows:write`, `feedbackRecords:write`, `responses:write`) is re-confirmed
...
<AccordionGroup>
  ...
  <Accordion title="invalid_scope during sign-in">
    ...
    `responses:read` and `responses:write` are deliberately **not** in that list yet. They can be
    granted, but the server does not advertise them, so a client that asks for them by name is the
    one case where this error is expected rather than a misconfiguration.
  </Accordion>
  ...
  <Accordion title="Self-hosted: the browser can't complete sign-in / redirects fail">
    ...
    If the Formbricks container or pod cannot resolve that public origin when it verifies a token, set
    `MCP_OAUTH_JWKS_URL` to the internal `/api/auth/jwks` endpoint. Keep `WEBAPP_URL` and your auth base
    URL public: the internal override is only used for the server-side signing-key fetch. On an instance
    that still configures the auth URL under its older name, that is the one that has to stay public.
    `/api/auth/jwks` is correct only when no custom application subpath is configured. If Formbricks is
    mounted at `/formbricks`, use the complete internal URL, such as
