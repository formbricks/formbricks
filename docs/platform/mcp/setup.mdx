...
<Steps>
  ...
  <Step title="Authenticate">
    ...
    ```bash
    codex mcp login formbricks --scopes surveys:read,feedbackRecords:read,offline_access
    ```
...
(`surveys:write`, `feedbackRecords:write`) is re-confirmed periodically for safety.
...
<AccordionGroup>
  ...
  <Accordion title="invalid_scope during sign-in">
    ...
    with the scopes the server advertises (`surveys:read`, `surveys:write`, `feedbackRecords:read`,
    `feedbackRecords:write`, `offline_access`).
  </Accordion>
