...
<Steps>
  <Step title="Create a Custom HTML tag in GTM">
    1. Create a new tag with preffered name
    2. Tag Type: Custom HTML
    3. Paste the code below (replace `<your-environment-id>` and `<your-app-url>`)
...
| app-url        | string | URL of the hosted Formbricks instance. |
  ...
  <Step title="Test">
    ...
    3. Add `?formbricksDebug=true` to see test logs in console
  </Step>
</Steps>
...
<Steps>
  <Step title="Create GTM variables">
   ...
   6. Save and publish
   7. Repeat for "userEmail" and "userPlan" (optional)
...
```
  ...
  <Step title="Create custom trigger">
    1. Create a custom event trigger in GTM
    2. Trigger Type: Custom Event
    3. Event name: `formbricksTrack` or name that matches with your event in code.
...
- Wait 5 minutes for cache (or use debug mode)
