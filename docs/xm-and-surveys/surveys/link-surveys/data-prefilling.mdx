...
You can prefill ranking questions with comma-separated values. The order of the values determines the ranking order.

**Using option labels** (backward compatible):

```sh Ranking Question with Labels
https://app.formbricks.com/s/clin3yxja52k8l80hpwmx4bjy?ranking_id=First%20Choice,Second%20Choice,Third%20Choice
```

**Using option IDs** (recommended):

```sh Ranking Question with Option IDs
https://app.formbricks.com/s/clin3yxja52k8l80hpwmx4bjy?ranking_id=choice-abc,choice-xyz,choice-123
```

<Note>
  You can find the option ID in the Survey Editor when you click on the option settings. Option IDs are stable even when you change the option label text.
</Note>

## Using Option IDs for Choice-Based Questions

All choice-based question types (Single Select, Multi Select, Ranking, Picture Selection) now support prefilling with **option IDs** in addition to option labels. This is the recommended approach as it's more reliable and doesn't break when you update option text.
...
- **CTA type questions:** Valid values are "clicked" (main CTA) and "dismissed" (skip CTA).
...
- **Ranking questions:** Values are comma-separated option IDs or labels. For required ranking questions, all choices must be included.
