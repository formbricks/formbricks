...
We also offer a way to generate personal links via the API. You can use the endpoint for generating links for [individual contacts](/api-v2-reference/management-api--surveys--contact-links/get-personalized-survey-link-for-a-contact) and for a [whole segment](/api-v2-reference/management-api--surveys--contact-links/get-survey-links-for-contacts-in-a-segment).
