...
export const handleFeedbackSubmit = async (YesNo, pageUrl) => {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_FORMBRICKS_COM_API_HOST}/api/v1/client/workspaces/${process.env.NEXT_PUBLIC_FORMBRICKS_COM_WORKSPACE_ID}/responses`, // For testing, replace this with the API host and workspace ID of your Development workspace on app.formbricks.com
  };
      {
        method: "POST",
        headers: {
...
export const updateFeedback = async (freeText, responseId) => {
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_FORMBRICKS_COM_API_HOST}/api/v1/client/workspaces/${process.env.NEXT_PUBLIC_FORMBRICKS_COM_WORKSPACE_ID}/responses/${responseId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
```tsx Replace the ID and API accordingly
  ...
  try {
    const res = await fetch(
      ...
      `https:app.formbricks.com/api/v1/client/workspaces/clgwcwp4z000lpf0hur7uxbuv/responses`,
  };
```
...
```tsx Replace the ID and API here as well
  try {
    const res = await fetch(
		// Note that we also updated the API host to 'https://app.formbricks.com/'
      `https:app.formbricks.com/api/v1/client/workspaces/clgwcwp4z000lpf0hur7uxbuv/responses/${responseId}`, // Note that we also updated the API host to 'https://app.formbricks.com/'
}
```
