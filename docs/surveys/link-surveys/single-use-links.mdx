...
Every single-use link is your normal survey link plus a **single-use ID** (`suId`) and a
**signature** (`suToken`) that binds it to one specific survey:
...
Both are bound to their survey by the same signature. The difference is who chooses the ID and
...
| `suToken` in the URL | Required. An HMAC signature over the survey ID and `suId` | Required. An HMAC signature over the survey ID and `suId` |
...
<Warning>
  Both modes are signed with your instance's `ENCRYPTION_KEY`, so respondents cannot forge a valid
  link by guessing an ID, and a link issued for one survey is not accepted by any other survey.
  They *can* forward their own link to someone else, so treat a single-use link as a credential.
</Warning>
...
With encryption on the `suId` is an encrypted blob instead. Either way each link carries its own
`suToken`.
...
- Links follow the survey's encryption setting. Each returned URL carries its own `suToken` in both
  modes.
...
- **Since Formbricks 6.0**, the same applies to *encrypted* single-use links: they now carry a
  `suToken` binding them to one survey. Encrypted links generated before 6.0 have no token and are
  rejected — regenerate them. See the
  [migration guide](/self-hosting/advanced/migration) if you handed out a batch before upgrading.
