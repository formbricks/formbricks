...
### Step 1: Add passed-in fields to the Formbricks survey

To be able to attribute a completed answer to a research participant, you need to add passed-in fields to your Formbricks survey. To do so, edit your survey and scroll down to the Embedded Data card.

Add three passed-in fields named `prolific_pid`, `study_id`, and `session_id`. Field names are lowercase, which is fine: Prolific sends its parameters in upper case and Formbricks matches them case-insensitively, so the values land in these fields either way.
