...
**In Settings → Workspace → Embedded Data**, a read-only **Auto-captured fields** card lists every one of them with its type, whether it can be used in logic and recall mid-survey, and what **Anonymize responses** does to it. It is the one place the whole catalog is visible without opening a response, and nothing on it is editable — every survey already has these fields.
