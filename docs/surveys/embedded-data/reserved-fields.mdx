...
**In exports**, the column set is derived from the catalog rather than from whatever the first response happened to carry, so it is stable for the survey: every response gets every column, and absent values export as empty. `Response ID`, `Survey ID`, `Finished` and the timestamp already have fixed basic columns, so they are not repeated. Neither privacy toggle removes a column: `Country`, `Browser`, `Os`, `Device Type` and `Ip Address` stay whether **Anonymize responses** is on or **Capture IP address** is off. Responses collected before you changed either setting keep their values, and responses that never captured a field export it empty. The response filter is the other way round — it only offers the fields the survey still captures.
