...
<Note>
  Values reach a response only through the fields the survey declares in its **Embedded Data** card, with the
  source set to **Passed in**. Declare the field first, then push a value under its key.
</Note>
