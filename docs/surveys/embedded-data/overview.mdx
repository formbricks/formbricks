...
| ------------ | -------------------------------------------- | ------------------------- | ------------------------------------- |
| **Ingested** | the outside world: URL params or the SDK     | Hidden Fields             | `string`, `number`, `boolean`, `date` |
...
- **In Settings → Workspace → Embedded Data**, for a field the whole workspace shares. A field declared there can be linked from any survey, and editing it once changes it for every survey using it. The same page lists which surveys use each field, and refuses to delete one that is still in use.
...
- **Hidden fields** declared in the survey editor are `string`. A shared field declared in workspace settings can be `string`, `number`, `boolean` or `date`, and incoming values are coerced to it.
...
<Note>
  Typing a field, giving it a default, and **locking** it so nothing outside the survey can write it are set
  on a shared field in Settings → Workspace → Embedded Data. A locked field ignores whatever a URL param or
  the SDK sends and uses its default instead, which is how you fix a value nobody outside can change.
</Note>
