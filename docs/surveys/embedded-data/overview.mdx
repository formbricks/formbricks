...
<Note>
  **The API payloads are unchanged** and no survey needs editing. What did change is the editor: the Hidden
  Fields and Variables cards are now one **Embedded Data** card, where a field's source is **Passed in** or
  **Calculated**. See [Hidden Fields and Variables](/surveys/embedded-data/migration) if you already use
  them.
</Note>
...
| ------------ | -------------------------------------------- | ------------------------- | ------------------------------------- |
| **Ingested** | the outside world: URL params or the SDK     | **Passed in**             | `string`, `number`, `boolean`, `date` |
| **Computed** | the survey itself, through logic actions      | **Calculated**            | `string`, `number`                    |
...
An ingested or computed field has to exist before anything can fill it, and there are two places to declare one:

- **In the survey editor**, for a field only that survey uses: the **Embedded Data** card, where one dialog creates either source.
- **In Settings → Workspace → Embedded Data**, for a field the whole workspace shares. A field declared there can be linked from any survey, and editing it once changes it for every survey using it. The same page lists which surveys use each field, and refuses to delete one that is still in use. See [Field library](/surveys/embedded-data/library).

Reserved fields are never declared, they are always available.

A passed-in field carries two things you type, and they do different jobs. Its **name** is what you call it — what the response table, the summary and your exports show, renameable at any time. Its **key** is the address the value lives under: the URL parameter, the SDK key, and what recall and logic resolve. The key fills itself in from the name, and cannot change once the field exists, because responses are already stored under it. A calculated field has a name only; its address is generated.

Keys follow one rule everywhere, and reserved names are refused for new fields in any casing. The full rule and the refused list live on the [Hidden Fields](/surveys/general-features/hidden-fields#naming) page.

A field also carries a **type** that decides how its value is read back and compared. In this release:

- **Passed-in fields** can be `string`, `number`, `boolean` or `date`, wherever they are declared, and incoming values are coerced to the declared type.
- **Calculated fields** are text or number — the types survey logic can produce — and the initial value you set is the default the field falls back to.
...
<Note>
  A field's type, its default, and the **lock** that stops anything outside the survey writing it are set in
  the same dialog, on a survey field and a [library field](/surveys/embedded-data/library) alike. A locked
  field ignores whatever a URL param or the SDK sends and uses its default instead, which is how you fix a
  value nobody outside can change.
</Note>
...
<Steps>
  ...
  <Step title="Coercion to the declared type">
    ...
    and `on` (and their negatives) become the strings `"true"` and `"false"` — that canonical pair is what a
    response stores, what an export column shows, and what a filter compares against; a `date` field accepts `2026-08-06` and full ISO 8601
    ...
    blocks a response. A field left as `string` — every field declared in the survey editor — passes through
    this step unchanged, since every scalar has a lossless text form; it does visible work on a shared field
    that declares a narrower type.
  </Step>
