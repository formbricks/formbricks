...
- A Formbricks account with your **Workspace ID** and **API host**, available under **Settings → Workspace → Web & Mobile SDK**, on the **Workspace details** card.
...
var t=document.createElement("script");t.type="text/javascript",t.async=!0,t.src=appUrl+"/js/formbricks.umd.cjs";t.onload=function(){window.formbricks.setup({workspaceId: workspaceId, appUrl: appUrl})};var e=document.getElementsByTagName("script")[0];e.parentNode.insertBefore(t,e)}();
</script>
...
Once you've completed the steps above, validate your setup by checking the **Connection status** card under **Settings → Workspace → Web & Mobile SDK**. The widget status indicator should change from this:

![The Connection status card, grey, reading "Formbricks SDK is not yet connected."](/images/surveys/website-app-surveys/framework-guides/widget-status-not-connected.webp)

To this:

![The Connection status card, green, reading "Receiving data" and "Formbricks SDK is connected"](/images/surveys/website-app-surveys/framework-guides/widget-status-connected.webp)
