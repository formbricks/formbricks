...
2. Choose **New field**, set the source to **Calculated**, give it a name, choose its type — text or number — and set a default value. Add as many as you need.

Each calculated field is addressed by an ID rather than by its name, shown on its row in the card, and that ID is what recall and logic use.
