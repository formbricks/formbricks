...
Each row says where the value comes from, what the field falls back to, and the address it is stored under. **Library** or **This survey** says which of the two declared it.

## Naming

A passed-in field has a **Name** and a **Key**, and only the Key is an identifier. The Name is free text — call the field `Plan tier` if that is what your team calls it — and you can rename it whenever you like. The Key is the URL parameter the value arrives under, and what recall and logic resolve; it fills itself in from the Name and cannot change once the field exists. Your **export column header is the Name**, so renaming a field renames its column.
