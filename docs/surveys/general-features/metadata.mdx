...
<Note>
  This metadata is the **reserved** half of [Embedded Data](/surveys/embedded-data/overview): every survey
  records it on every response without declaring anything, and you can use it in logic, recall, filters and
  exports by name. The full catalog, and the Anonymize toggle that suppresses the privacy-sensitive entries,
  are on [Reserved fields](/surveys/embedded-data/reserved-fields). The survey editor's **Embedded Data**
  card and **Settings → Workspace → Embedded Data** both list them alongside the fields you declare yourself.
</Note>
