...
<Note>
  This metadata is the **reserved** half of [Embedded Data](/surveys/embedded-data/overview): no survey has
  to declare it, and you can use it in logic, recall, filters and exports by name. Which entries actually
  carry a value depends on the survey type and its settings — see the note below. The full catalog, and the
  Anonymize toggle that suppresses the privacy-sensitive entries, are on [Reserved
  fields](/surveys/embedded-data/reserved-fields). The survey editor's **Embedded Data** card and **Settings
  → Workspace → Embedded Data** both list them alongside the fields you declare yourself.
</Note>
