...
| `feedbackRecords:read`  | `list_feedback_datasets`, `list_feedback_records`, `get_feedback_record` |
| `feedbackRecords:write` | `create_feedback_record`                                                    |
...
<Note>
  Restart the dev server after changing tool registrations. The MCP server is built once at module scope
  by `createMcpHandler`, so hot reload does not pick up an added, removed or renamed tool — `tools/list`
  keeps serving the previous set until the process restarts.
</Note>
...
<Note>
  ...
  (`surveys:read surveys:write feedbackRecords:read feedbackRecords:write offline_access`). Because the authorization server validates the
...
Output mirrors the Hub feedback-record shape and paginates with `meta.nextCursor`:
...
return `400` rather than guessing. A tenant id is never accepted from tool input, and
`get_feedback_record` additionally verifies the fetched record's tenant — the named directory when one
was given, otherwise any directory the workspace owns.
