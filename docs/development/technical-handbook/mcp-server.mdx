...
| `feedbackRecords:write` | `create_feedback_record`, `create_feedback_records`, `delete_feedback_record`                                                                                  |
...
`GET /v1/feedback-records` parameters:

| Filter            | Selects                                                              |
| ----------------- | -------------------------------------------------------------------- |
| `sourceType`      | a kind of source, e.g. `survey`, `review`, `call_notes`              |
| `sourceId`        | one survey/form/ticket                                               |
| `fieldType`       | one field type (`text`, `nps`, `rating`, …)                          |
| `fieldId`         | all answers to one question                                          |
| `fieldGroupId`    | one grouped question (ranking, matrix, grid)                         |
| `submissionId`    | the sibling answers given in one submission                          |
| `userId`          | everything one end user submitted                                    |
| `valueId`         | everyone who picked one particular choice                            |
...
<Note>
  ...
  record created moments ago is not searchable yet; `find_similar_feedback_records` reports that case as a
  retryable `409` rather than an empty result, and `search_feedback_records` simply won't match it yet.
</Note>
