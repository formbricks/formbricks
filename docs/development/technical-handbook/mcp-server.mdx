...
Strictness binds the top level only. The fields typed as free-form objects — `blocks`, `metadata`,
`welcomeCard` and friends on `create_survey`, and the `data` payload on `patch_survey` /
`validate_survey` — still accept any nested shape, and are validated by the v3 survey document contract
...
- `subscriptions/listen` is refused (`maxSubscriptions: 0`). The 2026-07-28 revision replaced the GET SSE
  ...
  A client that opens one gets `Subscription limit reached` and should fall back to polling the list
  endpoints.
