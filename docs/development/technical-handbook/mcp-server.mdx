...
<Note>
  ...
  feedbackRecords:write offline_access`). Because the authorization server validates the `/authorize`
  request against the client's **registered** scopes, a client that registers with a narrower set than
