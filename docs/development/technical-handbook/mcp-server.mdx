...
- Neither block tool validates that a reorder keeps `#recall:` references pointing backwards. The
  document validator checks that a recall target exists, never that it appears earlier in the flow, so
  a reorder can produce a survey that passes every server check and misbehaves at runtime. This
  predates the tools — whole-array replacement allows it too — but a one-call reorder makes it easier
  to reach.
