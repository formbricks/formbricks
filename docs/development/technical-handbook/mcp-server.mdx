...
**There are no exceptions, and that is now enforced rather than asked for.** The feedback-record tools
were one for a while — they gated inside their own handler factories and registered through
...
tool module at all. `registerScopedTool` is the only export of `guard-scopes.ts`, and a test pins that
— because the compiler can stop you forgetting the scope argument, but only that test stops someone
re-exporting a guard and hand-rolling the gate again.
