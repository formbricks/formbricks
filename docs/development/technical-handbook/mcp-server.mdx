...
- Ordering is validated as a delta: a change may not *introduce* a `#recall:` that points forwards, a
  logic condition reading a later block, or a `requireAnswer` targeting an earlier one
  (`misordered_reference`). Violations a survey already contains are left alone, so an existing survey
  never becomes unpatchable — enforcing the full set would repeat the ENG-3070 failure.
