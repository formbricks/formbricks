...
- Feedback records can be created, read, searched and deleted, but not updated: there is no
  `patch_feedback_record` tool. Records are treated as immutable submissions, and the enrichment fields
  (sentiment, emotions, translations) are derived by the Hub rather than written by callers.
