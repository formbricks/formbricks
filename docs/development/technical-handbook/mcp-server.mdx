...
| `feedbackRecords:read`  | `list_feedback_directories`, `list_feedback_records`, `get_feedback_record` |
...
| `list_feedback_directories` | `read`                                                    |
...
Lists the active feedback directories assigned to a workspace. Read-only and idempotent. Use the
returned `id` as `feedbackDirectoryId` for the other feedback-record tools.
...
Lists feedback records in a workspace's feedback directory. Read-only and idempotent, with opaque
cursor pagination. `feedbackDirectoryId` is optional when the workspace has exactly one active
directory (the common case) and required when it has more than one.
...
{
  "workspaceId": "clxx1234567890123456789012",
  "feedbackDirectoryId": "clfd1234567890123456789012",
  "limit": 50,
  "cursor": "opaque-cursor-from-previous-response",
  "sourceType": "survey",
  "fieldType": "text",
...
feedback directory assigned to the workspace; otherwise the tool returns a generic authorization
...
Creates a feedback record in a workspace's feedback directory. Writes data and is not idempotent, and
requires `feedbackRecords:write` (API keys need `write` or `manage`). The tenant is derived from the
resolved feedback directory and is never taken from the request body. `submission_id` is optional — a
...
A feedback record lives in the Formbricks Hub, addressed by an opaque `tenant_id`. Every tool
resolves that tenant server-side: it authorizes the caller's access to `workspaceId`, checks the
`feedbackDirectories` license, then maps the workspace to its assigned feedback directory — the
`FeedbackDirectory` id **is** the Hub `tenant_id`.

Assignment is a join table, but the application enforces at most one *non-archived* directory per
workspace, so in practice `feedbackDirectoryId` can be omitted and the single active directory is used.
It becomes required only if a workspace somehow has several active directories, in which case the tools
return `400` rather than guessing. `tenant_id` is never accepted from tool input, and
