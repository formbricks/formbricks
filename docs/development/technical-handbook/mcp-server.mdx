...
`feedbackRecords:read`, `feedbackRecords:write`, and `offline_access` — the scopes an MCP client
needs. Clients derive their Dynamic Client
...
| `delete_feedback_record`        | `write` or `manage`                                                 |
...
curl -sS -X POST http://localhost:3000/api/auth/oauth2/register \
  ...
  --data '{
    ...
    "response_types": ["code"],
    "scope": "openid profile email offline_access surveys:read surveys:write feedbackRecords:read feedbackRecords:write"
  }'
```
...
<Note>
  ...
  surveys:write feedbackRecords:read feedbackRecords:write offline_access`). Because the authorization server
  validates the `/authorize` request against the client's **registered** scopes, a client that registers with
  a narrower set than it later requests (e.g. it registers surveys-only but requests `offline_access`) is
  rejected with `invalid_scope`. To smoke-test the real path, omit `scope` and let the client adopt the
  advertised set, or pass exactly what the metadata advertises.
</Note>
...
codex mcp login formbricks-local \
  --scopes openid,profile,email,offline_access,surveys:read,surveys:write,feedbackRecords:read,feedbackRecords:write
```
...
```bash
codex mcp login formbricks-local \
  --scopes openid,profile,email,offline_access,surveys:read,feedbackRecords:read
```
...
(`surveys:read surveys:write feedbackRecords:read feedbackRecords:write offline_access`), and launches
the browser-based OAuth flow — there is no scope-entry step. You approve the requested scopes on the
Formbricks consent screen. Workspace role determines whether write tools actually succeed at execution time (see
...
`feedbackRecords:write`. Only the fields sent are changed; at least one is required.
...
`destructiveHint: true`, and requires `feedbackRecords:write` (API keys need `write` or `manage`). The
...
- The MCP server exposes the survey and feedback-record operations listed on this page.
- Survey tool coverage depends on the current v3 Surveys REST endpoint coverage; feedback-record tool
  coverage depends on the Hub feedback-records API.
