...
table and uses `tenant_id` to separate rows. For XM v5, the authoritative tenant boundary is the workspace
ID, so every Cube query must be scoped to the authenticated workspace before data leaves Cube.
...
emits a structured audit log line from `queryRewrite` with tenant, workspace, organization, user, request ID,
source, and queried member names. Raw filter values are intentionally omitted from both logs.
