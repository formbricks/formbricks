...
An unchanged chart-managed database needs no extra value. If an upgrade changes the database URL, credentials,
or source Secret, perform a two-step update:

1. Create or update a stable database Secret outside the Formbricks Helm release and wait until Kubernetes can
   read it.
2. Point `migration.database.existingSecret`, `authzed.activation.database.existingSecret`, and the application
   `DATABASE_URL` at that Secret, then run the Helm upgrade.

The Secret must contain `migration.database.urlKey` (default `DATABASE_URL`). It may also contain
`migration.database.migrateUrlKey` (default `MIGRATE_DATABASE_URL`); when that optional key is absent, migrations
use the runtime URL. Never introduce, rename, or rotate this Secret in the same Helm operation as a pre-upgrade
migration. The chart rejects custom database environment sources that do not name the database Secret
explicitly.
