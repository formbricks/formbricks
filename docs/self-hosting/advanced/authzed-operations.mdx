...
- The permanent Formbricks chart receives the private SpiceDB runtime and the immutable v5 bridge image.
...
receipt, schema, authorization contract, or client configuration is incompatible. There is no boolean
acknowledgement and no supported bypass. Schema mutation, graph repair, authority switching, and rollback never run
as hooks in the permanent chart.
...
Argo CD renders Helm templates with install semantics rather than setting `.Release.IsUpgrade`. Pause Argo CD or
Flux reconciliation before execution and pass `--gitops-paused` only after confirming the pause. The assistant
sets `authzed.activation.installBootstrap.enabled=false` for the bridge rollout; the temporary coordinator
establishes the receipt, and the application startup check remains the final fail-closed gate. Resume GitOps only
after the assistant restores the intended permanent release and removes the temporary coordinator.
