...
database bootstrap, the activation bootstrap, and finally the mandatory receipt check. Application Pods are not
replaced unless the final check succeeds. If an atomic install removed the release, Helm uses ordinary fresh
install semantics instead. Recovery cannot convert a populated v5 database: activation refuses a nonempty graph
without the migration receipt. Return `retryOnUpgrade` to `false` after a successful recovery.
