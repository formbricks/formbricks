...
been proven complete. Use the signed upgrade assistant distributed with the v6 release. It verifies signatures
and image digests, detects the existing deployment shape, and produces a reviewable plan before making changes.
...
For Helm, operators use two deliberately separate charts after the assistant's read-only preflight:
...
The Docker executor automates the equivalent sequence. Helm operators advance the temporary chart explicitly:
...
5. Finalize the receipt, uninstall the temporary chart, and retain the exact bridge image for the rollback window.
...
Argo CD renders Helm templates with install semantics rather than setting `.Release.IsUpgrade`. Existing Argo
applications must set `authzed.activation.installBootstrap.enabled=false` for the bridge rollout. The temporary
coordinator establishes the receipt, and the application startup check remains the final fail-closed guard even
though Argo does not execute Helm's pre-upgrade lifecycle.
