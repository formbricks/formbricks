...
Fresh-install and upgraded v6 processes wait for a compatible database activation receipt before initially
serving traffic. That bounded startup check reads PostgreSQL and makes no SpiceDB RPC. After startup, the general
Formbricks `/health` endpoint and Kubernetes readiness and liveness probes do not depend on SpiceDB. This prevents
a SpiceDB outage from restarting unrelated Formbricks workloads, but also means `/health` alone cannot prove that
authorization data is healthy.
...
| `AUTHZED_DATABASE_PASSWORD`             | Operator-owned password for Docker's dedicated `spicedb` role       | No default                                          |
| `AUTHZED_DATABASE_PASSWORD_URL_ENCODED` | Generated URI-safe form of the same Docker password                 | No default                                          |
...
The activation receipt is bound to `AUTHZED_ENDPOINT`, `AUTHZED_SYSTEM_KEY`, and the authorization contract.
Changing either value under an existing receipt fails startup instead of trusting a reachable but potentially
empty or stale datastore. Keep a stable private service or DNS name. An application-visible endpoint or namespace
change needs a fenced authoritative-to-authoritative refresh and is not an in-place configuration change; keep
the old target active and contact Formbricks support before attempting one.

The token and TLS mode may rotate in place at the same endpoint. First use the release-matched operations image
with the proposed credentials to verify health, schema, and a full clean relationship audit. Then update and
restart every Formbricks instance, repeat those checks, and retire the old credential only after all instances are
healthy. A backend replacement behind the same stable endpoint additionally requires stopping all mutation-serving
Formbricks instances while the new datastore is initialized, reconciled, and audited; otherwise source changes can
be acknowledged by the old datastore and be absent from the replacement.

For Docker, test the proposed token or TLS values with the `authzed-ops` service before recreating Formbricks. For
Helm, use the same release image in a short-lived, operator-managed Pod with the proposed Secret references. Keep
the old target and credentials available until the restarted application passes the same checks.
...
For Docker, the operator owns the raw `AUTHZED_DATABASE_PASSWORD` credential and the bootstrap consumes it.
Compose uses
`AUTHZED_DATABASE_PASSWORD_URL_ENCODED` only to interpolate that same credential into the SpiceDB datastore URI;
it is not an independently managed secret. `openssl rand -hex 32` is already URL-safe, so generated installations
write the same value to both variables. For a custom password containing URI-reserved characters, percent-encode
the companion and rotate both values together.
...
`authzed-initialize` is an idempotent, one-shot service that prepares fresh installations. Initial Formbricks
startup waits for the database receipt it creates, without connecting to SpiceDB during the wait. After startup,
`/health` remains independent from SpiceDB. The `authzed-ops` profile is a short-lived operator container using
the same release image and never starts during a normal Compose run.
...
run the activation bootstrap before reporting success. Existing v5 installations use the signed Docker
executor, which preserves the customized base Compose file and adds a permanent AuthZed overlay. It stops the
application only after the bridge is healthy and a database-backed preparation receipt has been created.
For this upgrade path, the release manifest pins both the PostgreSQL bootstrap and SpiceDB multi-architecture
images by digest. The executor writes `AUTHZED_POSTGRES_BOOTSTRAP_IMAGE_REF` and `SPICEDB_IMAGE_REF` before
rendering the overlay; tag-only or unexpected repositories are rejected.
...
The v6 assistant treats the cluster-scoped `SpiceDBCluster` CRD as shared platform state. With
`authzed.mode=selfHosted` and `authzed.operator.install=true`, it may create the CRD only when it is absent and
the operator identity has explicit `get` and `create` permission. It validates the CRD before creation and after
any concurrent creator wins. It never applies, patches, replaces, upgrades, or deletes an existing CRD. For a
shared or externally managed operator, set `authzed.operator.install=false`; a compatible, established
`v1alpha1` CRD must already exist. With `authzed.mode=external`, also keep operator installation disabled: the
chart and assistant create no SpiceDB operator or CRD and use the configured endpoint and named Secrets.
...
The one case this cannot repair is a `spicedb` role that already exists and was created by _someone else_,
...
### Keep pre-upgrade migrations on the intended database

The Helm migration Job reads only `WEBAPP_URL`, `DATABASE_URL`, and the optional `MIGRATE_DATABASE_URL`; it does
not inherit the application's broad `deployment.envFrom`. This prevents a pre-upgrade hook from depending on a
candidate Secret, ExternalSecret, or ConfigMap that Helm has not applied yet.

An unchanged chart-managed database needs no extra value. Do not change the database URL, credentials, or source
Secret in the same operation as a Formbricks version upgrade. Normalize the database configuration first:

1. Create a stable database Secret outside the Formbricks Helm release, update the currently running release to use
   it, restart the application, and verify normal database reads and writes. If the endpoint itself changes, finish
   its separate data-migration and cutover procedure in this step.
2. In a later version upgrade, point `migration.database.existingSecret`,
   `authzed.activation.database.existingSecret`, and the application `DATABASE_URL` at that already-active Secret.

Set `deployment.env.DATABASE_URL` as a non-optional `valueFrom.secretKeyRef`; this explicit EnvVar overrides every
`deployment.envFrom` source. The chart requires its Secret and key to match the activation Job and requires the
migration Job to use that same runtime Secret and key. The Secret may also contain
`migration.database.migrateUrlKey` (default `MIGRATE_DATABASE_URL`); when that optional key is absent, migrations
use the runtime URL. Helm verifies the references, not Secret contents. Never introduce, rotate, rename, or repoint
this Secret during the version upgrade. Rendering fails when custom database environment sources cannot be proven
to select this contract.

The runtime database URL used by activation must allow at least two concurrent connections. If it has an explicit
`connection_limit`, set it to `2` or higher. PgBouncer and other proxies must provide at least two usable database
connections for each activation process in addition to normal application capacity. Activation fails closed when
the URL declares a lower limit; the chart and assistant cannot prove the proxy's backend capacity, so verify that
...
migrations. A fresh Helm installation runs a bounded, idempotent `activation bootstrap` Job after the datastore
and Formbricks migrations become available. Formbricks startup waits for the resulting database receipt, but the
wait performs no SpiceDB RPC and general health probes remain independent from SpiceDB after startup.
...
kubectl exec -n formbricks deployment/formbricks -- formbricks-authzed activation runtime-check
```
...
### Recover an interrupted fresh Helm install

A fresh install can create its Secrets, databases, and migration resources but fail before the activation Job
writes the empty-install receipt. Do not disable the receipt gate. Correct the original failure and rerun the
same chart and values with an immutable image digest and the explicit recovery switch:
...
On a failed release this renders ordered `pre-upgrade` hooks: Formbricks migrations, the idempotent SpiceDB
database bootstrap, the activation bootstrap, and finally the mandatory receipt check. Recovery uses
`authzed.activation.installBootstrap.recoveryTimeoutSeconds` (20 minutes by default), which can outlive an
abandoned 15-minute mutation fence before safely resuming. Application Pods are not replaced unless the final
check succeeds. If an atomic install removed the release, Helm uses ordinary fresh install semantics instead.
Recovery cannot convert a populated v5 database: activation refuses a nonempty graph without the migration
receipt. Return `retryOnUpgrade` to `false` after a successful recovery.
...
been proven complete. Use the signed upgrade assistant distributed with the v6 release. Verify the bundle's
Sigstore signature first; the assistant then rechecks signed asset names and digests, detects the existing
deployment shape, and produces a reviewable plan before making changes.

Begin with the [signed v6 upgrade assistant](/self-hosting/advanced/v6-upgrade-assistant). Its read-only
preflight detects the installation and current version, verifies that the release manifest contains immutable
bridge and target image digests, and reports missing prerequisites without changing the deployment.

For Helm, the assistant uses two deliberately separate charts after its read-only preflight:

- The permanent Formbricks chart receives the private SpiceDB runtime and an immutable release-matched
  `legacy_bridge` image. It is built for the v6 cutover but keeps the PostgreSQL-backed v5 evaluator authoritative.
- The temporary `formbricks-upgrade` chart owns preparation, audit, fencing, activation, finalization, and rollback
  coordination. Uninstall it after finalization; no temporary cutover Job remains in the permanent release.

The candidate and released v6 application use the `spicedb_authoritative` runtime contract and require the
compatible activation receipt. Stable v5 releases remain `legacy_bridge`; stable v6 releases are
`spicedb_authoritative`. Release automation fails closed for an unknown future major and does not advance moving
source or image aliases until that major's publication and upgrade policy is explicitly reviewed.

The signed assistant advances both charts and records the original revision, values, replica count, HPA state,
signed asset identities, and database receipt in a local mode-`0600` journal. Its automated sequence is:

1. Deploy the bridge with `authzed.activation.upgradeGate.enabled=false`. PostgreSQL remains authoritative for
   decisions while the durable projection is populated. This is the only documented Helm upgrade without the
   pre-upgrade receipt gate, and the bridge image must still be pinned through `deployment.image.digest`.
2. Apply the release schema, drain the projection outbox, reconcile the complete relationship graph, and create a
   database-backed receipt tied to the bridge and candidate image digests.
3. Quiesce authorization mutations, verify a clean final audit and monotonic outbox watermark, then atomically
   activate the receipt.
4. Upgrade the permanent release to the digest-pinned v6 candidate with `migration.mode=external` and the
   pre-upgrade receipt gate enabled. The candidate image validates the database receipt before replacing Pods;
   the exact bridge image already applied the release's shared expand migrations.
5. Verify the candidate image, authorization runtime, and container-local application health before finalizing.
6. Restore the intended replica/HPA state and migration policy, verify the rollout, uninstall the temporary chart,
   and retain the exact bridge image for the rollback window.

Run `execute --install-type helm` with the two checksum-verified local chart archives. If the process is
interrupted, rerun the identical signed inputs with `resume`. The assistant consults the database activation state
as well as its journal, so a running v6 bridge or candidate is never mistaken for a completed upgrade. See the
[v6 Upgrade Assistant](/self-hosting/advanced/v6-upgrade-assistant) for complete commands.

The candidate gate runs `formbricks-authzed activation runtime-check`. It reads PostgreSQL only and fails when the
receipt, schema, authorization contract, or required decision invariants are incompatible. This binding remains
active after cutover: compatible patch images may change, but a release with a new authorization contract or
canonical schema requires a supported, fenced authoritative-to-authoritative refresh procedure before it can
start. Do not bypass this fail-closed gate. Token and TLS mode may rotate in place at the same endpoint using the
verified transport-change procedure above. Endpoint and system-namespace changes require a new fenced receipt.
Schema mutation, graph repair, authority switching, and rollback never run as hooks in the permanent chart.

<Warning>
  The first v6 candidate rollout must use `migration.mode=external`; the permanent release's bridge-image
  migration Job applies the release's rollback-compatible migrations while legacy authorization still serves
  traffic. Do not let a candidate migration Job race the authority switch.
</Warning>

The automated Kubernetes upgrade requires a real Helm release visible through `helm list` and `helm get values`.
Pause any controller reconciling that release and pass `--gitops-paused` only after confirming the pause. Argo CD
normally renders charts without creating a Helm release; those installations are not eligible for the automated
path and must remain on v5 until a support-reviewed or controller-native migration is available. The assistant
sets `authzed.activation.installBootstrap.enabled=false` for the bridge rollout, and application startup remains
...
<Steps>
  ...
  <Step title="Verify the activation receipt">

    ```bash
    formbricks-authzed activation runtime-check
    ```

    Do not start a direct-authority release unless this exits `0`. Existing upgrades obtain the receipt from the
    temporary coordinator; fresh installations create it through the install bootstrap.
...
The automated rollback releases its authority fence only after the bridge reports the `legacy` runtime contract
and its container-local `/health` check passes. If either proof fails, keep traffic fenced and follow the
assistant's manual-recovery result.
...
`umask 077` and remove any earlier export first. The umask governs file _creation_ only, so under a typical
