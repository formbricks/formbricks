...
<Warning>
  The first v6 candidate rollout must use `migration.mode=external`; the temporary coordinator applies the
  release's rollback-compatible migrations while the bridge still serves traffic. Do not let a normal migration
  Job race the authority switch.
</Warning>
