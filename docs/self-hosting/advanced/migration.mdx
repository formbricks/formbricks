...
**seven database migrations**, all applied automatically on startup: six are quick — new columns and
tables, plus two data migrations that run on their own — and one builds an index on the `Response` table
that blocks response writes while it builds — see [Database Migrations](#database-migrations) before
scheduling the upgrade.

v5.4 also upgrades the **Better Auth** authentication core to 1.7. That upgrade needs no action at all:
every migration is additive and applies automatically, and **your SSO callback URLs do not change** — see
below.

### Database Migrations

All seven migrations apply automatically on startup and none needs a manual step — but one of them
deserves a scheduling decision on large instances, so here is the full list rather than a summary:
...
| `add_organization_display_time_zone` | Adds the nullable `Organization.displayTimeZone` used for response-export timestamps. | Instant |
...
| `eng_2343_backfill_oauth_resource_links` | Data migration linking existing MCP OAuth clients, refresh tokens and consents to this instance's `${WEBAPP_URL}/api/mcp` resource, so already-registered MCP clients survive the upgrade. No-op on a fresh install. | Automatic |
...
<Warning>
  ...
  How much this costs you depends on how you deploy. On Kubernetes the chart runs migrations in their own
  job while the pods already running keep serving traffic, so responses arriving during the build are the
  ones that block. On Docker Compose the `formbricks-migrate` service has to finish before the app starts,
  and the documented upgrade stops the app first — nothing is serving, so the same build shows up as a
  longer restart rather than blocked writes.
</Warning>
...
v5.4 use the same callback URLs.
