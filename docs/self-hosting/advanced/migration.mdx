...
- **BETTER_AUTH_URL** and **BETTER_AUTH_SECRET** are now the documented names for the auth base URL and
  ...
  Formbricks now refuses to start when neither variable has a value. If your instance set neither — it
  could still start before this release, and only invites and verification links failed — generate a
  secret before upgrading. Note that doing so **signs every current user out**: their cookies were
  signed with a fallback value, not with a secret of yours.
