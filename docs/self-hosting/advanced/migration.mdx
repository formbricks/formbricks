...
- **BETTER_AUTH_URL** and **BETTER_AUTH_SECRET** are now the documented names for the auth base URL and
  ...
  Formbricks now refuses to start when neither variable has a value, rather than starting and then
  failing on the first invite or verification link it tries to send.
