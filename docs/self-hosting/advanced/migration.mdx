...
tables, two of them data migrations that run on their own — and the seventh builds an index on the
`Response` table that blocks response writes while it builds — see
...
| `add_organization_display_time_zone` | Adds the nullable `Organization.displayTimeZone`, which formats response timestamps both in file exports and in the payloads pushed to third-party integrations. | Instant |
...
after rolling forward — this is the same statement the `repair_account_issuer` migration runs, and it is
idempotent:
...
### Better Auth 1.7 upgrade (no action required)
...
automatically on startup — they are the two `eng_2343` entries in the
