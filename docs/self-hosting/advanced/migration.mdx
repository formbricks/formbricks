...
Regenerate and re-send the links for any survey with single-use enabled and **URL encryption of
single-use ID** on, either from the survey's share modal or via
`GET /api/v1/management/surveys/{surveyId}/singleUseIds`. Formbricks never stores generated links,
so there is nothing to migrate and nothing to back up — new links simply replace the old ones.
...
Because outstanding links stop working the moment the new version serves traffic, prefer a
maintenance window: a respondent who opened an encrypted link before the deploy and submits after it
will see the invitation error and lose their answers.
