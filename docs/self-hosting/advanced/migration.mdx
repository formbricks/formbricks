...
the Feedback Records API or the MCP server. It needs no environment variables and no database migrations.
If you do not use Unify Feedback, or you only reach it through the UI, there is nothing to do for it.
...
This change adds no migrations and touches no schema, so rolling the application images / Helm values back
to v5.3 undoes it with nothing to restore. An API key raised to `manage` in step 2 keeps working on v5.3 —
