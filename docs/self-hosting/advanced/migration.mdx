...
- **BETTER_AUTH_URL** and **BETTER_AUTH_SECRET** are now the documented names for the auth base URL and
  ...
  One edge case worth checking before you upgrade: if your configuration defines `BETTER_AUTH_SECRET`
  with an **empty** value, remove the line (or give it a real value). An empty value now counts as unset
  and falls through to `NEXTAUTH_SECRET`, which signs users out once and requires any registered MCP
  OAuth clients to be re-registered. Formbricks refuses to start if neither variable has a value.
...
<Note>
  ...
  set `BETTER_AUTH_URL` to a different origin, the callback lives on **that** origin instead — it is
  derived from the same auth base URL Better Auth itself uses, not from `WEBAPP_URL`.
...
- `BETTER_AUTH_SECRET` — cookie/session signing secret. Falls back to `NEXTAUTH_SECRET` when unset. If you
  set it explicitly, use the value `NEXTAUTH_SECRET` already holds: setting a *different* one invalidates
  every session and every outstanding invite and verification link. At least 32 characters is recommended.
