...
1. Back up the customized Compose file, then download the file from the release you are upgrading to. Use that
...
To roll back only this change, stop the worker and restore the backed-up Compose file. Completed jobs remain
committed and pending jobs remain queued, but all Hub asynchronous processing pauses again. A fully custom
deployment that runs Hub workers outside this Compose project must use its own manual upgrade workflow rather
than the one-click updater.
