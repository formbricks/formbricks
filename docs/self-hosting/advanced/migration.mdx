...
**seven database migrations**, all applied automatically on startup: six are quick, additive changes, and
one builds an index on the `Response` table that blocks response writes while it builds — see
...
| --- | --- | --- |
| `add_feedback_source_import_mode` | Adds `FeedbackSource.import_mode`, which controls whether a source's historical import includes partial responses. Existing sources keep the current behaviour (`completedOnly`). | Instant |
...
All seven are additive and none of them is read by v5.3, so they do not constrain a rollback: rolling the
application images / Helm values back to v5.3 is safe without touching the database.
