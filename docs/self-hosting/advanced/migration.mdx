...
Use the signed v6 upgrade assistant rather than performing the authorization migration by hand. It validates the
current release, renders the exact image digests and Secret references, and drives the temporary upgrade chart.
The customer-owned Helm values retain only the permanent AuthZed runtime after the coordinator is removed.
...
2. Deploy the immutable legacy-authoritative v5 bridge with
   `authzed.activation.upgradeGate.enabled: false`; this creates the permanent SpiceDB resources and durable
   projection without changing authorization decisions.
3. Let the temporary coordinator apply the schema, drain projections, reconcile relationships, and record a
   database-backed activation receipt while the bridge remains available.
4. Quiesce authorization mutations briefly, activate that receipt, and deploy the immutable v6 candidate with
   `migration.mode: external` and `authzed.activation.upgradeGate.enabled: true`.
5. Let the candidate's pre-upgrade gate verify its embedded contract against the receipt before any application
   Pod is replaced.
6. Finalize, remove the temporary chart, and verify critical allow, deny, revocation, and cross-tenant cases.

There is no `migrationAcknowledged` flag: a stale or missing receipt cannot be bypassed by a values change. Keep
the exact bridge image and values until the rollback window closes. See
[AuthZed Operations](/self-hosting/advanced/authzed-operations#upgrade-an-existing-installation-to-v6) for the
assistant, backups, repair, cutover, and rollback procedure.
