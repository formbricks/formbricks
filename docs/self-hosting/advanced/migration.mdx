...
- **BETTER_AUTH_URL** and **BETTER_AUTH_SECRET** are now the documented names for the auth base URL and
  ...
  neither variable set still started, falling back to the authentication library's built-in default
  secret, and only invites and verification links failed. So a production instance is already required
