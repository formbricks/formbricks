...
- **Cube is feature-scoped**. It is only required when you use analytics dashboards or other analysis flows
  that depend on Cube queries.
...
Cube is not a blanket v5 requirement for every self-hosted deployment.

- configure Cube only if you use analytics dashboards or other analysis flows that depend on it
- the current production Docker Compose and Helm deployment assets do **not** provision Cube automatically
- when you do run Cube, the Formbricks app expects `CUBEJS_API_URL` and `CUBEJS_API_TOKEN`; the Cube service
  itself also needs its own Cube configuration such as `CUBEJS_API_SECRET`
...
<Tabs>
  <Tab title="One-Click">
    ...
    - configure Cube separately only if this instance needs analytics dashboards or analysis queries
  ...
  <Tab title="Docker Compose">
    ...
    - Cube is configured separately only if you need analytics dashboards or analysis queries
    ...
    <Info>
      The production Docker Compose stack bundles Hub, but it does not provision Cube for you.
    </Info>
  </Tab>
  <Tab title="Kubernetes">
    ...
    - configure Cube separately only if the instance needs analytics dashboards or analysis queries
  </Tab>
</Tabs>
...
- verify dashboards and analysis flows only if you configured Cube
