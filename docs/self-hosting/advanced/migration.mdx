...
| SAML (BoxyHQ / Jackson) | `https://your-domain.com/api/auth/callback/saml` | **No**, for connections created with the wildcard `redirect_uri` Jackson uses by default (`${WEBAPP_URL}/*`). Only re-register if you configured an explicit, non-wildcard `redirect_uri`. |
...
proxied through it too. No action is required — the route's only rate-limit rule is an `Exact` match on
`/api/auth/callback/credentials`, so SSO traffic is not newly throttled — but expect SSO callbacks to start
appearing in this proxy's logs and metrics where they did not before.
