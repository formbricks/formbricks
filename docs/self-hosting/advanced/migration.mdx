...
### Bundled TEI log level

The Helm chart now defaults `hub.embeddings.env.RUST_LOG` to `warn` because TEI 1.9 includes its API key in
INFO-level startup arguments. If you need a different log level, set `hub.embeddings.env.RUST_LOG` explicitly
and use it only with a runtime that redacts credentials from its logs.
...
### Encrypted single-use survey links must be regenerated (action required if you use them)

Encrypted single-use survey links issued before v6 stop working and have to be regenerated.

#### What to do when upgrading

**Regenerate after upgrading, not before.** Links minted by the old version are unsigned whichever
day you mint them, so regenerating first produces a second batch that the new version rejects exactly
like the first.

1. Plan a maintenance window covering the deploy and the redistribution. Outstanding links stop
   working the moment the new version serves traffic, and a respondent who opened one before the
   deploy and submits after it sees the invitation error and loses their answers.
2. Deploy v6.
3. Regenerate the links for any survey with single-use enabled and **URL encryption of single-use
   ID** on, from the survey's share modal or via
   `GET /api/v1/management/surveys/{surveyId}/singleUseIds`. The new version signs them.
4. Re-send. Formbricks never stores generated links, so there is nothing to migrate and nothing to
   back up — the new links simply replace the old ones.

**Links with URL encryption off are unaffected**, including custom single-use IDs. Those have carried
a `suToken` signature since 5.0 and validate exactly as before, so integrations that mint their own
IDs need no changes.

#### What you will see if you skip it

Opening an old encrypted link shows *"This survey can only be taken by invitation"*, and submissions
against it are rejected. Server logs record one `warn` per rejection carrying the survey id and a
`reason` of `missing_signature`.

#### Why

An encrypted single-use link carried only an encrypted id and no signature, so nothing tied it to the
survey it was issued for and any survey on the same deployment accepted it. Both modes now carry the
same signature, which is what the old links are missing.

#### If you cannot re-issue in time

`SINGLE_USE_LEGACY_UNSIGNED_UNTIL` keeps the old links working for a fixed period, so an in-flight
campaign does not die on upgrade day. It takes an ISO 8601 date and is unset by default:

```sh
SINGLE_USE_LEGACY_UNSIGNED_UNTIL=2026-11-15
```

Unsigned **encrypted** links are accepted until 00:00 UTC on that date, and rejected from then on.
Nothing else changes: links with URL encryption off are untouched, a `suToken` that is present but
wrong is still rejected, and the id must still decrypt under your `ENCRYPTION_KEY`.

<Warning>
  This re-opens the vulnerability the release closed, for as long as it is set. An unsigned encrypted
  link is bound to no survey, so a link issued for one survey opens **any** survey on the same
  instance — on a deployment serving more than one organisation, that is across organisations. Set it
  only on a single-tenant instance, only for as long as re-issuing takes, and only as a deliberate
  decision. It is not a substitute for step 3 above.
</Warning>

Each accepted legacy link is logged at `warn` with the survey id and the configured expiry, and
counted on `formbricks_survey_single_use_link_validations_total` with `reason="legacy_unsigned"`.
Watch that series fall to zero, then remove the variable — it tells you when the window has stopped
carrying traffic and re-issuing is complete.

#### Rollback

Downgrading restores the old links, since nothing about them changed on disk — but it also restores
the behaviour above, so treat it as a stopgap while you regenerate rather than a fix.
||||||| 1cb21762d6

### Hub worker required for Docker

Fresh Docker and one-click installations start `hub-worker` with the Hub API. The worker consumes every
PostgreSQL-backed asynchronous Hub job, including webhook deliveries and any configured enrichment. An existing
installation keeps its local `docker-compose.yml` during `formbricks.sh update`, so merge the worker before
updating; the updater now exits before pulling images, running migrations, or stopping services when the
resolved Compose project does not contain `hub-worker`.

1. Start in the directory containing your customized `docker-compose.yml`, then back it up and download the file
   from the release you are upgrading to. For a one-click installation, run `cd formbricks` first. Use that
   release's tag instead of `stable` when your deployment is pinned.

   ```bash
   cp -p docker-compose.yml docker-compose.yml.before-hub-worker
...
2. Compare the two files. Merge `x-hub-runtime-environment` and the release's `hub` and `hub-worker` sections as
   one unit so the API and worker retain the same database and taxonomy configuration. Preserve local proxy,
   storage, database, and image-pin customizations. `hub`, `hub-worker`, and `hub-migrate` must resolve to the
   same Hub image reference.
3. Validate the result before changing the running stack:

   ```bash
   docker compose config >/dev/null
   docker compose config --services | grep -Fx hub-worker
   ```

4. For a one-click installation, rerun `./formbricks.sh update`. For a manually managed Compose installation,
   apply Hub migrations before starting the API and worker from the new image:

   ```bash
   docker compose pull hub hub-migrate hub-worker
   docker compose up --force-recreate hub-migrate
   docker compose up -d hub hub-worker
   ```

5. Verify that the worker stays running and that the existing River backlog decreases:

   ```bash
   docker compose ps hub-worker
   docker compose logs --tail=200 hub-worker
   ```

The worker has no HTTP health endpoint. A stable process and decreasing queue are the readiness signals. If it
restarts, verify PostgreSQL connectivity and every configured enrichment provider credential. Jobs already in
River remain stored in PostgreSQL and begin draining when the worker starts; no repair or re-enqueue is required.

To roll back only this change, remove the worker container before restoring the backed-up Compose file so its
`restart: always` policy cannot start it again:

```bash
docker compose rm -fs hub-worker
cp -p docker-compose.yml.before-hub-worker docker-compose.yml
```

Completed jobs remain committed and pending jobs remain queued, but all Hub asynchronous processing pauses
again. A fully custom deployment that runs Hub workers outside this Compose project must use its own manual
upgrade workflow rather than the one-click updater.
...
<Tabs>
  <Tab title="One-Click">
    ...
    - include the bundled `formbricks-migrate`, `hub-migrate`, `hub`, and `hub-worker` services
  ...
  <Tab title="Docker Compose">
    ...
    - the compose stack includes `formbricks-migrate`, `hub-migrate`, `hub`, and `hub-worker`
