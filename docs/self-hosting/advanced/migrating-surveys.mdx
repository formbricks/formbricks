...
| `403` on a create, after the same key read the source fine                          | The key has read but not write on the **target** workspace, or the target workspace was created after the key | Create a **new** key with the target workspace selected and `write`/`manage` — a key's workspace access cannot be edited after creation |
