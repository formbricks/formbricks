...
<Steps>
  <Step title="Create an API key on each instance">
    ...
    to — see [Generate API Key](/api-reference/generate-key). Workspace access is granted per
    workspace and there is no "all workspaces" option, so a key cannot reach a workspace that was
    created after it: add the new workspace to the key's Workspace Access list explicitly, or every
    write returns `403`. Read access is enough on the source; the target needs write or manage.
  </Step>
...
| `403` on a create, after the same key read the source fine                          | The key has read but not write on the **target** workspace, or the target was created after the key and never added to its Workspace Access list | Edit the key and give it `write`/`manage` on the target workspace |
