...
# --- Audit logging ---
AUDIT_LOG_ENABLED=1
AUDIT_LOG_PATH=/var/log/formbricks/audit.log   # change if needed
REDIS_URL=redis://redis:6379                   # existing Redis instance
```

2. Redeploy your containers.
3. Confirm you can see a new file at the path specified by `AUDIT_LOG_PATH` after performing any action in the UI/API.

<Hint type="info">Audit logs rotate **daily** or when the file reaches **100 MB**, and up to **90 days** of compressed history is kept by default. Adjust the `logrotate` policy if your compliance rules differ.</Hint>
...
- **Scope limitation:** For now, **only events triggered inside the Formbricks application** are audited. This means:
  ...
  - **Survey responses** created via API or client-side SDKs are **not** audited.
