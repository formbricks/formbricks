...
- **Description:**
  - Formbricks blocks new sign-ups from personal / free / disposable email domains (`gmail.com`, `yahoo.com`, `outlook.com`, `mailinator.com`, …) by default. On the sign-up form the address is rejected with "Please sign up with your company email address."
  - Set this to `1` to accept personal addresses.
  - The check applies wherever a **new** account would be created: the first admin on a fresh instance, SSO just-in-time provisioning (see `AUTH_SKIP_INVITE_FOR_SSO`), and the sign-up form where it is reachable. Existing users are never affected — they can always log in. Invited users are exempt unless `SIGNUP_DOMAIN_CHECK_ON_INVITES=1`.
- **Use case:**
