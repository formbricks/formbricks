...
Invite limits count successfully created or refreshed invites. A bulk request for 25 people consumes 25 units
only when all 25 invites are created; recipients who are already members, already invited, or otherwise fail do
not consume the daily budget. Self-hosters can change the limit with `INVITE_RATE_LIMIT_PER_24_HOURS`;
Formbricks Cloud organizations with the `bulk-invite` entitlement receive a limit of 500 recipients.
