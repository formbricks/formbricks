...
<Warning>
  **Upgrading from an earlier version?** This path changed at v5.2 and reverted at v5.4:

  - v5.1 and earlier, and v5.4 and later (current): `{WEBAPP_URL}/api/auth/callback/openid`
  - v5.2 through v5.3: `{WEBAPP_URL}/api/auth/oauth2/callback/openid`

  If you followed the v5.2 migration guide and left the pre-v5.2 URI registered in your Keycloak
  client's **Valid redirect URIs**, there is nothing to do — v5.4 simply starts using it again. Only add
  it back if you removed it after upgrading to v5.2. See the
...
<AccordionGroup>
  ...
  <Accordion title="Authentication fails with a redirect error">
    ...
    - If you're on v5.2 or v5.3, note the path is `{WEBAPP_URL}/api/auth/oauth2/callback/openid` (with `/oauth2`) instead — v5.4 reverts to the URI above.
