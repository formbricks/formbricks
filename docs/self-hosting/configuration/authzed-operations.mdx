---
title: "SpiceDB Authorization Operations"
description: "Configure, monitor, back up, and repair the SpiceDB authorization dependency used by Formbricks."
icon: "shield-halved"
---

Formbricks uses [SpiceDB](https://authzed.com/docs/spicedb), maintained by AuthZed, to store a relationship
graph for authorization. PostgreSQL remains the source of truth. Formbricks projects membership, team,
workspace, and API-key changes into SpiceDB after their PostgreSQL transaction commits. Existing environment
variables and operator commands keep the `AUTHZED_` prefix for compatibility.
...
| Formbricks operator commands | Check health and schema state, then audit or repair relationship drift          |
...
The v6 server validates required AuthZed configuration before accepting requests, without making an RPC.
The general Formbricks `/health` endpoint, application startup, and Kubernetes readiness and liveness probes
do not depend on SpiceDB connectivity. This prevents a SpiceDB outage from restarting unrelated Formbricks workloads. It
...
v6 migration. The updater runs only the read-only `upgrade check` before stopping the existing application.
A blocked check leaves it running. After a successful check, the updater stops the stack, waits for PostgreSQL,
runs application migrations, and starts the new stack only if they succeed. Preparation is a separate
[maintenance operation](/self-hosting/advanced/v6-maintenance-upgrade), including for later schema-changing releases.
...
For existing releases, the pre-upgrade hook is read-only. Prepare schema and relationship changes explicitly
before the upgrade; the hook only runs `upgrade check`.
...
Formbricks v6 has no legacy authorization fallback. Follow the
[maintenance upgrade guide](/self-hosting/advanced/v6-maintenance-upgrade): stop all writers and controllers,
take recoverable backups, and run the target release's database migrations before preparing its graph.
A bridge release is not required for the self-hosted maintenance path.

The release-matched preparation command coordinates migrations and these existing operations; advanced
operators can also run them explicitly while maintenance remains active:
...
Keep the original v5 images, configuration, coordinated backups, and schema digest until acceptance is complete.
After database migrations, restoring an old image or running `helm rollback` alone may be incompatible with the
database. Keep maintenance active and either fix forward or restore the matching backups and v5 configuration.
Do not disable AuthZed inside v6. See the [recovery steps](/self-hosting/advanced/v6-maintenance-upgrade#results-and-recovery).

Use explicit release versions and image digests for this upgrade. Do not rely on a moving `latest` or `stable`
tag to perform the migration or to provide a tested rollback artifact.
...
The v6 projection layer writes projection intent in the same PostgreSQL transaction as an authorization source
change. BullMQ wakes the delivery worker, but PostgreSQL remains the durable queue. Deletes, and updates that are
not provably grants, are treated as revocations; direct authority fails closed when a revocation remains
unresolved for 60 seconds or enters dead letter.
...
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Missing AuthZed configuration at startup                                                 | Required v6 settings are absent, disabled, or use weaker consistency | Set the five required client settings, retain the appropriate TLS mode, and restart; there is no legacy fallback |
| `authzed_disabled` in a diagnostic CLI                                                   | That command loaded disabled client configuration     | Configure the required v6 client settings before starting the server |
