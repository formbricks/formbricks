...
delivery holds a worker slot for up to `WEBHOOK_DELIVERY_TIMEOUT_MS` (5 seconds by default). Deployments with
many webhooks per survey, or with slow receivers, should raise `BULLMQ_WORKER_CONCURRENCY` so a burst of
deliveries does not queue behind the pipeline and workflow jobs.
...
network errors, timeouts, DNS failures, redirects, 408, 429 and 5xx statuses are retried. Deliveries to a
webhook that was deleted or unsubscribed from the event while the job was queued complete without a request.
