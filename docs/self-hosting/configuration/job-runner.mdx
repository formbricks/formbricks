...
<Note>
  ...
  `SURVEY_SCHEDULING_TIME_ZONE` — updates the schedule as soon as the process restarts, but takes effect
  from the **following** run: the run already queued keeps its previous timing. For the daily sweeps that
  can be up to 24 hours. Inspecting the schedule in Redis shows the new value immediately, so a job firing
  once more at the old hour after a restart is expected rather than a sign the setting was ignored.
</Note>
