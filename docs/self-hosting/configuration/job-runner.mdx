...
| `Dropping BullMQ job with no registered processor` for `webhook-delivery.process`                   | Seen only while upgrading to the first version that fans webhook delivery out into its own job: replicas still on the older build share the queue but do not know the name, so they complete the job **without delivering it** — the event is not retried and cannot be replayed. It stops as soon as every replica runs the new build. Upgrade during a quiet period if losing a few minutes of webhook events matters, or scale to a single replica for the upgrade. |
