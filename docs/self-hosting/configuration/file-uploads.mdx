...
description: "Configure file storage for survey images, file uploads, and Workspace assets in your self-hosted Formbricks instance"
icon: "upload"
---
...
RustFS supports CORS two ways: a **server-level** allow-list set via the `RUSTFS_CORS_ALLOWED_ORIGINS` environment variable on the RustFS server, and **per-bucket** CORS via the S3 `PutBucketCors` API (e.g. `rc bucket cors set`).

**Bundled Formbricks setup** (the [one-click script](/self-hosting/setup/one-click), driven by `docker/rustfs-init.sh`): CORS is configured for you. The bootstrap applies a per-bucket policy via `rc bucket cors set`, scoped to your Formbricks domain — nothing else to do.
...
<Warning>
  ...
  per-bucket `rc bucket cors set` is verified to work.)
</Warning>
