...
candidate Secrets and ConfigMaps are updated. If an upgrade changes the database URL, credentials, or source
Secret, first create or update a stable database Secret outside the release and wait for it to become available.
Then configure `migration.database.existingSecret`, `authzed.activation.database.existingSecret`, and the
application's `DATABASE_URL` to use that Secret before starting the Helm upgrade. The Secret may also contain an
optional `MIGRATE_DATABASE_URL`; never introduce or rotate it in the same release operation as the migration Job.
...
key, transport, and consistency contract. When `deployment.env` or a custom `deployment.envFrom` selects the
application database, also set `authzed.activation.database.existingSecret` to the matching database Secret.
