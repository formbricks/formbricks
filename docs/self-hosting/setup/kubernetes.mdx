...
Use `authzed.mode: external` plus `authzed.endpoint` when connecting to an externally managed AuthZed endpoint.
The application endpoint remains internal by default when the chart owns SpiceDB.
