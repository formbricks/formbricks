...
retries the empty-install activation, and still runs the mandatory receipt gate before replacing Pods. Use this
only to recover a failed fresh install and return it to `false` after success; it is not an existing-installation
upgrade path.
