...
into the Helm pre-upgrade gate, which verifies the receipt before replacing application Pods. The gate stays
disabled for ordinary tag-based upgrades and requires `deployment.image.digest` when enabled; application startup
always performs the bounded database-only check.

The default `migration.mode: job` is appropriate for fresh installs and ordinary compatible upgrades. Use
