...
1. **Start the Docker Setup**
   ...
   PostgreSQL, Redis, the Formbricks Hub API and worker, Cube, and SpiceDB.
...
1. **Verify the Stack**
   ...
   docker compose logs --tail=100 formbricks hub hub-worker cube spicedb
   ```

   `formbricks-migrate`, `hub-migrate`, `authzed-db-bootstrap`, and `spicedb-migrate` should complete
   successfully. `postgres`, `redis`, `cube`, `hub`, `hub-worker`, `spicedb`, and `formbricks` should be running
   or healthy. The worker has no HTTP health endpoint, so verify that it stays `Up` without repeated restarts.
...
services:
  ...
  rustfs-init:
    ...
    entrypoint:
      ...
      - |
        set -e
        until rc alias set rustfs http://rustfs:9000 "$RUSTFS_ADMIN_USER" "$RUSTFS_ADMIN_PASSWORD" >/dev/null 2>&1 \
...
```bash
docker compose logs --tail=200 formbricks-migrate hub-migrate formbricks hub hub-worker cube
...
- **Hub auth errors**: Confirm the same `HUB_API_KEY` from `.env` is used by Formbricks Web and the Hub
  service.
- **Hub jobs remain queued**: Confirm `docker compose ps hub-worker` reports `Up`, then inspect
  `docker compose logs --tail=200 hub-worker`. The worker has no HTTP health endpoint; a stable process and a
  shrinking River backlog are its readiness signals. If it restarts, confirm each configured enrichment
  provider/model has the required credentials and that the worker can reach PostgreSQL.
