...
<Info>
  Feedback Records are served by the Formbricks application at `/api/v3/feedback-records`, so a reverse
  proxy needs no special wiring for them — forward the path like any other. The application authenticates the
  caller, checks the dataset, and talks to Hub itself, so Hub stays internal and its API key never leaves the
  application.

  The older arrangement still works if you already run it: gateway auth against the Formbricks app, a rewrite
  from `/api/v3/feedbackRecords` to Hub's `/v1/feedback-records`, and `Authorization: Bearer <HUB_API_KEY>`
  injected only on the Hub-bound hop. It is **deprecated** and will be removed in a later release; there is no
  need to set it up on a new install.
</Info>
