...
<Info>
  ...
  proxy needs no special wiring for them — forward the path like any other. The application authenticates
  the caller, checks the dataset, and talks to Hub itself, so Hub stays internal and its API key never
  leaves the application. Earlier releases required the proxy to run gateway auth, rewrite the path and
  inject Hub's credential; that is no longer the case, and configuration doing it should be removed.
</Info>
