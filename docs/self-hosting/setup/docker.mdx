...
its credentials, uses fully-consistent authorization, and exposes SpiceDB on `127.0.0.1:50051`. Set the mode
to `external` to preserve an existing external client configuration and skip the bundled SpiceDB services.
External mode requires enabled, fully-consistent AuthZed configuration. To start the optional authenticated
browser UI for bundled mode:
