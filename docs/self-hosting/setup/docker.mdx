...
1. **Create the Environment File**
   ...
   Keep `POSTGRES_PASSWORD` unchanged after PostgreSQL initializes the `postgres` volume. If you replace an
   existing `docker-compose.yml`, copy the password from that deployment instead of generating a new one; the
   PostgreSQL image only applies this value when it creates a new database volume.
