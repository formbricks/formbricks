...
1. **Create the Environment File**
   ...
   install, replace both URL values with your public HTTPS URL before starting the stack. Copy the immutable
   community image reference from the release metadata; the placeholder below must be replaced with its full
   SHA-256 digest.
   ...
   Keep `FORMBRICKS_IMAGE_REF` identical for the application, migration, and AuthZed operation containers; the
   Compose file rejects a missing value so those processes cannot drift between releases. Keep
