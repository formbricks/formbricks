...
1. **Create the Environment File**
   ...
   install, replace both URL values with your public HTTPS URL before starting the stack. Fresh stable
   installations can omit `FORMBRICKS_IMAGE_REF` and use `ghcr.io/formbricks/formbricks:stable`. For an
   immutable deployment or prerelease, copy the full SHA-256 image reference from the release metadata.

   ```bash
   POSTGRES_PASSWORD=$(openssl rand -hex 32)
   AUTHZED_DATABASE_PASSWORD=$(openssl rand -hex 32)
   cat <<EOF > .env
   # Optional for stable. Uncomment with a real digest for immutable deployments or prereleases.
   # FORMBRICKS_IMAGE_REF=ghcr.io/formbricks/formbricks@sha256:<release-digest>
   ...
   AUTHZED_DATABASE_PASSWORD_URL_ENCODED=$AUTHZED_DATABASE_PASSWORD
   EOF
   chmod 600 .env
   ```

   Keep `FORMBRICKS_IMAGE_REF` identical for the application, migration, and AuthZed operation containers. If
   it is omitted, all four use the `stable` alias together so those processes cannot drift. Keep
   `POSTGRES_PASSWORD` unchanged after PostgreSQL initializes the `postgres` volume. The generated
   ...
   `AUTHZED_DATABASE_PASSWORD` is the operator-owned credential that the bootstrap uses for the dedicated
   `spicedb` role. `AUTHZED_DATABASE_PASSWORD_URL_ENCODED` is only the generated form used to interpolate that
   credential into a database URI; it is not a second password. The hexadecimal value above is URL-safe, so both
   variables use the same value. If you choose a password containing reserved characters, percent-encode the
   companion and rotate both values together.
...
before the initial Formbricks process serves traffic. Startup waits for the database activation receipt, but the
bounded wait performs no SpiceDB RPC. Once Formbricks has started, `/health` remains independent from SpiceDB.
SpiceDB is reachable only inside the Compose network at `spicedb:50051`; it is not published on the host or
...
`AUTHZED_DATABASE_PASSWORD` private and include `.env` and the PostgreSQL volume in your backup plan. Activation
requires the Formbricks `DATABASE_URL` to permit at least two concurrent connections. Set an explicit
`connection_limit` to `2` or higher; if PgBouncer or another proxy is used, reserve at least two usable database
connections for the activation process in addition to normal application capacity. Running `docker compose down`
preserves the volume; `docker compose down -v` deletes both Formbricks and SpiceDB data.
...
Repository development defaults to `FORMBRICKS_DEV_AUTHZED_MODE=bundled`; `pnpm db:up` generates and preserves
its credentials, starts the datastore, applies the Formbricks database migrations, and creates the durable
fresh-install activation receipt before returning. It uses fully-consistent authorization and exposes SpiceDB
on `127.0.0.1:50051`. Set the mode to `external` to preserve an existing external client configuration and skip
the bundled SpiceDB services. External mode requires enabled, fully-consistent AuthZed configuration and the
same empty-database bootstrap will initialize it. To start the optional authenticated browser UI for bundled
mode:
...
any `AUTHZED_*` value. Token and TLS rotation at the same endpoint do not require a new activation receipt. The
endpoint and system namespace are receipt-bound and cannot be changed in place. Follow
[AuthZed Operations](/self-hosting/advanced/authzed-operations#configure-authzed) before any credential,
transport, or datastore replacement.
...
Do not manually merge services into a customized v5 Compose file or use the old boolean acknowledgement. Back
up the installation and PostgreSQL, download and verify every asset from the target v6 release, then use the
receipt-backed Docker executor documented in the
[v6 Upgrade Assistant](/self-hosting/advanced/v6-upgrade-assistant#execute-a-docker-or-one-click-upgrade).

The executor installs a signed permanent overlay beside the customer-owned Compose file, generates the two
stable AuthZed secrets, builds and audits the graph through the exact bridge image, then performs a guarded
authority switch. A candidate failure automatically restores the recorded bridge. Bundled PostgreSQL is
supported by the automated path; external PostgreSQL requires the operator runbook because its administrative
credentials cannot be derived safely.

The bridge and candidate are separate release-matched runtime modes. `legacy_bridge` keeps the PostgreSQL-backed
v5 evaluator authoritative while SpiceDB is populated and audited; `spicedb_authoritative` is the v6 runtime and
starts only with a compatible activation receipt. Stable v5 images remain bridge-compatible, while stable v6
images are SpiceDB-authoritative. Publication fails closed for an unknown future release major: no moving image
or source alias advances until that major's upgrade policy is explicitly defined.
...
<Warning>
  **Security (existing installs):** these steps — and the [one-click setup
  script](/self-hosting/setup/one-click) updater, which runs the same `pull` / `down` / `up -d` — do **not**
  rewrite your existing `docker-compose.yml`. If your bundled `redis` (Valkey) service still publishes `ports:
  - "6379:6379"`, it stays exposed to the host after updating. See [Debug](#debug) for how to remove the
  mapping and, if the port was reachable from an untrusted network, invalidate existing sessions.
</Warning>
