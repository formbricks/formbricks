...
- **v3 Responses API Metrics** — the names below are the exported series names through both the Prometheus exporter and an OTLP collector:
  ...
  - `formbricks_api_v3_responses_unresolved_responses_total` and `formbricks_api_v3_responses_unresolved_entries_total` — responses served with a non-empty `unresolved[]`, and its entries by `reason`. These count every *read*, so a client polling one unchanged response raises them without any survey edit, and `valueShapeMismatch` can be legacy malformed data rather than a definition change. Read the share of affected reads as `unresolved_responses_total` over `reads_total` by `operation` (`list` or `get`), and use `unresolved_entries_total` grouped by `reason` for the breakdown, rather than a rising raw count.
