...
- **v3 Responses API Metrics** — the names below are the exported series names through both the Prometheus exporter and an OTLP collector:
  ...
  - `formbricks_api_v3_responses_read_duration_seconds` — wall-clock seconds per read, by `operation`, `via` and `status_class`
  - `formbricks_api_v3_responses_page_surveys` — how many distinct surveys one list page spans, i.e. the size of its single survey query; the first bucket boundary is `0.5`, so an empty page is distinguishable from a one-survey page
