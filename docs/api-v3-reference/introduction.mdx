...
`404` you can see comes from the store, on `/{id}/similar` for a record that exists but has no embedding yet. Any
other path the gateway receives under `/api/v3/feedbackRecords` answers `400` — only the eight documented
operations exist.
