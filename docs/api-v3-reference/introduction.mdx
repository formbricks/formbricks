...
API v3 is the management API the Formbricks app and the [MCP server](/platform/mcp/overview) are built on. It
covers surveys, workflows, action classes, contact attribute keys and — the reason most beta users are here —
**Feedback Records**, the normalized store behind Unify Feedback.
...
[Generate API key](/api-reference/generate-key) — and send it in the `x-api-key` header. Keys carry per-workspace permissions — `read`, `write` or
`manage` — and every operation states which one it needs.
...
| Layer | Statuses | Body |
| ---------------- | --------------------------------------------- | ------------------------------------------------------------ |
| Gateway refusals | `400`, `401`, `403`, `413`, `503` | `text/plain` — a constant message such as `Forbidden` |
| Feedback store | `400`, `409`, `500`, `503` | `application/problem+json` with a stable `code` to branch on |

The gateway never answers `404`: an unknown dataset or record id is a `403`, so ids cannot be probed. The one
`404` you can see comes from the store, on `/{id}/similar` for a record that exists but has no embedding yet. Any path
under `/api/v3/feedbackRecords` that is not one of the eight documented operations answers `400`.
